  ...
  try {
    ...
    try {
      MCO.setServerURIs.overload('[Ljava.lang.String;').implementation = function (uris) {
        try {
          if (uris !== null) {
            for (var i = 0; i < uris.length; i++) log('mqtt', '[LEAD->broker] PAHO setServerURIs=' + uris[i]);
          }
          ...
          ov.implementation = function () {
            try {
              ...
              if (arguments.length > 0 && arguments[0] !== null && ('' + arguments[0]).indexOf('://') > -1) {
                log('mqtt', '[LEAD->broker] ' + cn.split('.').pop() + ' serverURI=' + arguments[0] + '  (穿透: 调证该 host:port 服务器归属)');
              }
            } catch (e) { skip('PAHO/broker', e); }
        ...
        try {
          if (C.serverHost) {
            C.serverHost.overload('java.lang.String').implementation = function (h) {
              try { log('mqtt', '[LEAD->broker] HiveMQ ' + cn.split('.').pop() + '.serverHost=' + h); } catch (e) { skip('HIVEMQ/broker', e); }
        ...
        try {
          if (C.serverPort) {
            C.serverPort.overload('int').implementation = function (p) {
              try { log('mqtt', '[LEAD->broker] HiveMQ ' + cn.split('.').pop() + '.serverPort=' + p + '  (穿透: serverHost:serverPort 即 broker)'); } catch (e) { skip('HIVEMQ/broker', e); }
