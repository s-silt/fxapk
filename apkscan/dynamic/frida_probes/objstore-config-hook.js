...
// 例(合成 bucket 名，仅示范 _bucketOf 的最左段提取；厂商域后缀须保留真实字面才能说明识别结构):
//     examplebucket0001.oss-accelerate.aliyuncs.com -> bucket=examplebucket0001   // leak-scan: allow 公有云厂商域，示范 bucket 提取结构，非案件值
//     examplebucket0002.gz.bcebos.com               -> bucket=examplebucket0002   // leak-scan: allow 公有云厂商域，示范 bucket 提取结构，非案件值
//     examplebucket0003.jiangsu-10.zos.ctyun.cn     -> bucket=examplebucket0003   // leak-scan: allow 公有云厂商域，示范 bucket 提取结构，非案件值
//     examplebucket0004.cos.ap-shanghai.myqcloud.com -> bucket=examplebucket0004  // leak-scan: allow 公有云厂商域，示范 bucket 提取结构，非案件值
...
function _noteObjStore(url, where) {
    try {
        ...
        if (_once('objstore|' + url)) {
            console.log('\n[objstore][' + prov + '][OBJSTORE锚点][LEAD->对象存储] (' + where + ') ' + url);
    ...
    try {
        ...
        for (var i = 0; i < keys.length; i++) {
            if (_once('backend|' + keys[i])) {
                console.log('[objstore][BACKEND][LEAD->真后端] 下发配置正文里的真后端线索: ' + keys[i] + '   <- 来自 ' + srcUrl);
