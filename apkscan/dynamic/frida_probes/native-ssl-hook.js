    ...
    Interceptor.attach(sslWrite, {
      onEnter: function (a) {
        try {
          ...
          // ★回灌专用独立行：只含裸 ip:port 锚点、绝不含明文（preview 明文那行不进回灌，防解析污染+公开面外泄）。
          var pbw = peerBare(a[0]); if (pbw) console.log('[ssl][LEAD->TLS对端] ' + pbw + '  (SSL_write ' + label + ')');
      ...
      onLeave: function (ret) {
        try {
          ...
          if (n > 0 && this.buf && !this.buf.isNull()) {
            ...
            var pbr = peerBare(this.ssl); if (pbr) console.log('[ssl][LEAD->TLS对端] ' + pbr + '  (SSL_read ' + label + ')');
          }
