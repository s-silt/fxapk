...
★ 定位：境外归属阶段（``phase="overseas"``）的第三方数据 enricher（``active=False``）。它不向
被查域名建立连接，但会把域名提交给公共 CT 日志库 crt.sh。免 key：本模块产出关联主机名/子域候选；
共享证书、通配证书和多租户关系必须排除，不能据此确认 Origin、家族或运营者。

为什么有用（关联候选价值）：
- CT 日志（Certificate Transparency）公开记录了为
  ``*.{domain}`` / ``sub.{domain}`` 签发的证书，含其 ``name_value``（SAN / 覆盖的子域）与
  ``issuer_name``（签发 CA）。本次查询返回的证书名称可形成历史/当前关联候选，但不保证全量覆盖；
- 这些名称与 Shodan ``hostnames`` 互补，可召回历史关联候选；但证书关系不是控制关系，须用独立
  技术或运行时锚点复核；
- 本组件不向候选子域发起连接或探测，但调用 crt.sh 本身会披露查询域名。
...
合规：只查公共 CT 日志，不向被查域名或候选子域发起连接 / 扫描 / 利用；查询 crt.sh 会向其披露域名。
"""
