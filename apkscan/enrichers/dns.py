...
def _classify_a_records(payload: dict) -> tuple[list[str], list[dict[str, str]]]:
    """抽取并校验 A 记录，把代理 Fake-IP/基准测试地址隔离。

    ``198.18.0.0/15`` 是 RFC 2544 基准测试网段，也被多种透明代理用作 Fake-IP 池；它不能
    作为公网基础设施候选进入托管归属。其它私网/保留地址仍保留原值，由更上层按场景判断（例如
    sinkhole/内网配置本身可能是证据），本层只处理已确认会伪造公网归属的这一段。
    """
    ips: list[str] = []
    ...
    if isinstance(answers, list):
        for ans in answers:
            ...
            try:
                ...
                rejected.append({"value": raw.strip(), "reason": "invalid_ip_literal"})
                continue
            value = str(addr)
            if addr.version == 4 and addr in _BENCHMARK_NET:
                rejected.append({"value": value, "reason": "benchmark_or_proxy_fake_ip"})
                continue
            if value not in ips:
                ips.append(value)
    return ips, rejected
...
def _extract_a_records(payload: dict) -> list[str]:
    """向后兼容 helper：只返回可进入归属链的 A 记录。"""
    return _classify_a_records(payload)[0]
...
def _resolve_doh(domain: str) -> tuple[list[str], list[str], list[dict[str, str]], dict[str, Any]]:
    """逐个 DoH 提供方解析并返回值、拒绝项与解析器来源，首个 HTTP 成功即采用。
    ...
    for url in DOH_URLS:
        try:
            ...
            ips, rejected = _classify_a_records(payload)
            return ips, _extract_cnames(payload), rejected, {
                "method": "doh",
                "resolver": url,
                "queried_at": time.time(),
                # 本函数为预算/隐私只采用首个成功视角；正式互证由 close/人工复核补足。
                "independent_views": 1,
...
def _resolve_socket(domain: str) -> tuple[list[str], list[str], list[dict[str, str]], dict[str, Any]]:
    ...
    **规范名(第一元素)常就是 CDN 落点**（``api.foo`` → ``api.foo.w.kunlungr.com``，此时 aliases
    ...
    payload = {"Answer": [{"type": _DNS_TYPE_A, "data": a} for a in addrs if a]}
    ips, rejected = _classify_a_records(payload)
    return ips, cnames, rejected, {
        "method": "system",
        "resolver": "system:gethostbyname_ex",
        "queried_at": time.time(),
        "independent_views": 1,
        "cross_validated": False,
    }
...
class DnsEnricher(BaseEnricher):
    ...
    def enrich(self, ep: Endpoint) -> EnrichmentResult:
        ...
        if not ips:
            try:
                socket_ips, socket_cnames, socket_rejected, socket_resolution = _resolve_socket(domain)
                ips = socket_ips
                cnames = socket_cnames or cnames
                rejected_answers.extend(
                    item for item in socket_rejected if item not in rejected_answers
                )
        ...
        if not ips:
            if rejected_answers:
                # Fake-IP/基准测试答案本身是有价值的解析环境证据。返回成功但明确“无可用公网 A”，
                # 让 pipeline 落盘 rejected_answers；不做 hosting、也不缓存，避免把代理环境的
                # 合成阴性永久固化。
                data: dict[str, Any] = {
                    "ips": [],
                    "hosting": [],
                    "resolution": resolution,
                    "resolution_status": "no_usable_public_a",
        ...
        data: dict[str, Any] = {"ips": ips, "hosting": hosting, "resolution": resolution}
        if rejected_answers:
            data["rejected_answers"] = rejected_answers
        if cnames:
