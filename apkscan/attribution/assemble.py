...
def _bridge_endpoint(endpoint: Any) -> tuple[list[AttributionEvidence], list[dict[str, Any]]]:
    ...
    if domain is not None:
        ...
        for source_key in ("certs", "shodan"):
            block = provider_payload_if_hit(enrichment, source_key)
    ...
    if kind == "ip":
        ...
        for source_key in ("asn", "shodan"):
            asn = _parse_asn(provider_payload_if_hit(enrichment, source_key).get("asn"))
            if ip is not None and asn is not None:
                edges.append(_evidence(
    ...
    if (
        ...
        and _domain_in_observed_sni(domain, runtime)
    ):
...
class NetworkObservations:
    ...
    def from_endpoint(
        ...
        return cls(
            ...
            asn_country=provider_payload_if_hit(enrichment, "asn").get("country"),
...
def _ip_endpoint_country(ep: Any) -> str | None:
    ...
    c = provider_payload_if_hit(enr, "asn").get("country")
    return c if isinstance(c, str) and c else None


def _endpoint_asn_country(ep: Any) -> str | None:
    """端点自身 asn 富化里的国家（仅采信非空字符串）。绝不抛。"""
    c = provider_payload_if_hit(_as_dict(getattr(ep, "enrichment", None)), "asn").get("country")
    return c if isinstance(c, str) and c else None
