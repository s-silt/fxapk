...
列设计（10 列）::
  ...
  - is_c2           = 按 advice/category 与显式 case_evidence 重新投影的派生标注；不盲信磁盘缓存
...
def leads_to_ioc_rows(report: dict[str, Any], only_investigate: bool = False) -> list[dict[str, Any]]:
    ...
    if not isinstance(raw_leads, list):
        return []
    # Keep reference-only values visible in the normal all-rows export, but
    # project them to 待核/non-C2 before filtering or rendering.  Thus
    # ``--only-investigate`` cannot turn batch/legacy material into a case IOC.
