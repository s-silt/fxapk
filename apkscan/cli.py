...
def config_channel_cmd(
    prefix: str = typer.Option(..., "--prefix", help="算法前缀常量（样本里抠出的 ENABLE_SETTING 类常量，如 BW-755-）。"),
    domain: str = typer.Option(..., "--domain", help="基域（OSS_URL 类常量，如 sdfkjn755fvb.com）。"),
    path: str = typer.Option("", "--path", help="配置对象路径（如 /Swuus_osschat.txt）；空则只到域名。"),
