...
def probe_leads(
    ...
    首批 8 个探针已脱敏后随包内置于 `apkscan/dynamic/frida_probes/`，其余自备。
...
def leak_scan_cmd(
    ...
    七条判据：公网 IP、疑似凭据、域名、语境框架词，以及三条**案件值**判据——
    中文人名（「<姓名>案」语境）、联系方式（QQ / 微信 / Telegram）、二开改包样本包名。
    ...
    默认阻断的是精确、误报可控的那几条：``ip`` / ``secret`` / ``person_name`` /
    ``contact`` / ``package`` / ``exemption``；``domain`` / ``context`` 天生噪音大，
    如实报告但不变红，``--strict`` 才升级为阻断。

    ★案件值判据（人名 / 联系方式 / 包名）的 finding **只报形态不回显原值**：公开仓库的
    CI 日志同样公开，把真名、QQ 号原样打进 Actions 输出等于换个地方泄漏。要看具体是什么，
    打开 finding 指出的那一行。
