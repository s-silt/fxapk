...
def score_asset(endpoint: Any) -> AssetScore:
    ...
    direct_evidences = [
        ev
        for ev in (getattr(endpoint, "evidences", None) or [])
        if getattr(ev, "scope", EvidenceScope.LEGACY_UNSPECIFIED)
        is EvidenceScope.CASE_EVIDENCE
    ]
    sources = {str(getattr(ev, "source", "")) for ev in direct_evidences}
    score = 0
    reasons: list[str] = []
...
def _has_business_path(endpoint: Any) -> bool:
    ...
    if not any(
        getattr(ev, "scope", EvidenceScope.LEGACY_UNSPECIFIED)
        is EvidenceScope.CASE_EVIDENCE
        for ev in (getattr(endpoint, "evidences", None) or [])
    ):
        return False
