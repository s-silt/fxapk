...
from apkscan.core.source_status import normalize_source_status, provider_payload_if_hit
...
def _ensure_source_status_coverage(
    ...
    for enricher in enrichers:
        ...
        # A missing provider entry is not the same thing as a recorded failure.
        # ``normalize_source_status(None)`` deliberately fails closed for malformed
        # report data, so only normalize when the provider was actually present.
        current = (
            normalize_source_status(raw_statuses[provider])
            if provider in raw_statuses
            else {}
        )
        if current:
            raw_statuses[provider] = current
...
def _resolved_ips(endpoint: Endpoint) -> list[str]:
    if endpoint.kind != "domain":
        return []
    dns = provider_payload_if_hit(endpoint.enrichment, "dns")
...
def _set_attribution(endpoint: Endpoint) -> None:
    from apkscan.core.attribution import build_domain_control, build_endpoint_attribution
    ...
    if attribution is not None:
        ...
        endpoint.enrichment.pop("attribution", None)
    if endpoint.kind == "domain":
        domain_control = build_domain_control(endpoint.value, endpoint.enrichment)
        if domain_control is not None:
            endpoint.enrichment["domain_control"] = domain_control
        else:
            endpoint.enrichment.pop("domain_control", None)
    else:
        endpoint.enrichment.pop("domain_control", None)
