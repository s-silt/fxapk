...
def _provider_payload(
    enrichment: Mapping[str, object], provider: str
) -> dict[str, Any]:
    """Return provider evidence only when its recorded outcome is admissible.

    Legacy reports without a provider outcome remain readable.  Once an
    outcome exists, only ``hit`` may contribute positive layer evidence;
    failed/skipped/disabled/no-record payloads are metadata, not proof.
    """

    return provider_payload_if_hit(enrichment, provider)


def _registration_layer(enrichment: Mapping[str, object]) -> dict[str, object]:
    rdap = _provider_payload(enrichment, "ip_rdap")
...
def _bgp_layer(enrichment: Mapping[str, object]) -> dict[str, object]:
    bgp = _provider_payload(enrichment, "ripestat_bgp")
    evidence = {
        key: bgp.get(key)
...
def _passive_hosting_evidence(
    ...
    fofa = _provider_payload(enrichment, "fofa")
    ...
    censys = _provider_payload(enrichment, "censys")
    ...
    virustotal = _provider_payload(enrichment, "virustotal")
...
def _hosting_layer(enrichment: Mapping[str, object]) -> dict[str, object]:
    ...
    asn = _provider_payload(enrichment, "asn")
    attribution = _attribution_for_endpoint(enrichment)
...
def _runtime_layer(endpoint: Endpoint) -> dict[str, object]:
    ...
    runtime_evidence = [
        ev
        for ev in endpoint.evidences
        if ev.scope is EvidenceScope.CASE_EVIDENCE and ev.source.startswith("runtime")
    ]
    evidence = {
        "sources": sorted({ev.source for ev in runtime_evidence}),
...
def _normalize_source_status(enrichment: Mapping[str, object]) -> dict[str, dict[str, object]]:
    return normalize_source_status_map(enrichment.get("source_status"))
...
def _aggregate_source_status(
    ...
    for target in resolved:
        ...
        if not isinstance(raw, Mapping):
            continue
        for provider, item in normalize_source_status_map(raw).items():
