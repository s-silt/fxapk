...
def _runtime_info(endpoint: Endpoint) -> dict[str, Any]:
    observed = any(
        ev.scope is EvidenceScope.CASE_EVIDENCE and ev.source.startswith("runtime")
        for ev in endpoint.evidences
    )
    if not observed:
        # Runtime enrichment has no independent scope marker.  Without a
        # directly qualified runtime Evidence it cannot license payload,
...
def _select_targets_with_stats(report: Report, max_targets: int) -> tuple[list[Endpoint], dict[str, object]]:
    ...
    lead_case_evidence: set[tuple[str, str]] = set()
    ...
    for endpoint in report.endpoints:
        ...
        if key not in lead_rank:
            continue
        # 批量/跨案参考与旧版未说明作用域的材料可以留在报告中供复核，但不能独立占用
        # 当前案件闭环目标名额。混合证据只要有一条 case_evidence 即可继续。
        if not has_case_evidence(endpoint.evidences) and key not in lead_case_evidence:
            scope_excluded += 1
            continue
    ...
    if scope_excluded:
        stats["scope_excluded"] = scope_excluded
