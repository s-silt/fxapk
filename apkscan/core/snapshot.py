...
仅 **android（APK）** 走并行快照；无 apk_path 由 pipeline 回退串行。
"""
...
class SnapshotContext:
    ...
    def __init__(
        ...
        *,
        ...
        # 脱壳 dump 的额外 .dex 文件路径（jadx 增强器在 worker 内读取、一并反编译）。曾漏带：
        # 消费方 `getattr(ctx, "extra_dex_paths", None) or []` 静默退化成空列表，串行生效、
        ...
        # 调用方断言为官方参照的 jadx 索引 key（同上纪律：必填过并行边界）。
...
def build_snapshot(ctx: Any) -> SnapshotContext:
    ...
    return SnapshotContext(
        ...
        jadx_baseline_index=getattr(ctx, "jadx_baseline_index", None),
...
if TYPE_CHECKING:
    ...
    # 静态结构校验（pyright，运行期零成本）：SnapshotContext 必须**完整**实现 AnalysisContext
    # 协议——含数据字段。协议加字段而快照漏带时，类型检查关直接变红，而不是等分析器在并行
    # worker 里用 getattr 兜底静默拿到空值（extra_dex_paths 曾如此漏带：串行生效、并行失效）。
    # 运行期的同口径闸在 tests/test_parallel_analyzers.py::test_snapshot_satisfies_analysis_context_protocol。
    _PROTOCOL_CHECK: type[AnalysisContext] = SnapshotContext
