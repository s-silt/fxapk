...
from apkscan.core.source_status import canonicalize_report_source_status
...
def _evidence_from_dict(value: object) -> Evidence:
    ...
    try:
        scope = EvidenceScope(str(value.get("scope", EvidenceScope.LEGACY_UNSPECIFIED.value)))
    except ValueError:
...
def report_from_dict(payload: Mapping[str, object]) -> Report:
    ...
    payload = migrate_report_payload(payload)
    leads: list[Lead] = []
    ...
    if isinstance(raw_meta, Mapping) and isinstance(raw_meta.get("closure"), Mapping):
        meta["closure"] = project_serialized_closure(payload)
    ...
    # External consumers see one canonical five-state provider contract even
    # when the persisted input was written by an older string-valued writer.
    # Canonicalize every endpoint (including resolved-IP children) at the load
    # boundary so no typed consumer can accidentally materialize the legacy
    # shape again.
    canonicalize_report_source_status(report)
    # 旧版/外部报告可能已有“建议调证”物化档位；scope 迁移后立即按直接证据资格重算，
    # 避免 typed load → case close 这条路径继续沿用批次参考升级出来的旧档位。
    from apkscan.core.leads import quarantine_reference_only_leads

    quarantine_reference_only_leads(report.leads, report.endpoints)
    return report
...
def write_report(
    ...
    *,
    ...
    ensure_writable_report_version(report.schema_version)
