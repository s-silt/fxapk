...
def _licensed_attribution_enrichment(enrichment: dict[str, Any]) -> dict[str, Any]:
    """Copy attribution inputs after enforcing every tracked provider outcome.

    The copy deliberately drops ``source_status`` only *after* removing every
    provider payload that lacks a canonical hit.  This gives the domain path a
    safe working object into which it may add a licensed parent-DNS CNAME or
    hosting fallback without relabelling those derived facts as a child ASN/DNS
    query result.
    """
    licensed = dict(enrichment)
    for provider in _ATTRIBUTION_PROVIDER_SOURCES:
        payload = provider_payload_if_hit(enrichment, provider)
        if payload:
            licensed[provider] = payload
        else:
            licensed.pop(provider, None)
    licensed.pop("source_status", None)
    return licensed
...
def _online_as_org(enrichment: dict[str, Any]) -> str | None:
    ...
    for src in _ONLINE_ASORG_SOURCES:
        node = provider_payload_if_hit(enrichment, src)
        if not node:
            continue
...
def attribution_from_enrichment(enrichment: dict[str, Any], ip: str = "") -> dict[str, Any] | None:
    ...
    if not isinstance(enrichment, dict):
        return None
    enrichment = _licensed_attribution_enrichment(enrichment)
    ...
    if kind_s == "ip":
        ...
        for ip in ordered:
            ...
            if isinstance(resolved_ip, dict):
                merged = _licensed_attribution_enrichment(resolved_ip)
...
def build_domain_control(value: str, enrichment: dict[str, Any]) -> dict[str, Any] | None:
    """构建与服务器五层归因**并行**的域名控制链。

    链路为注册商 → 权威 DNS 产品 → zone/account → record operator。公开 RDAP/NS 最多能识别
    服务商和产品线；共享/Anycast NS 不能识别租户、账户辖区或实际操作者，这三项恒保持 unknown，
    等待经授权的服务商记录补齐。
    """
    if not isinstance(enrichment, dict):
        return None
    domain = _s(value).strip().lower().rstrip(".")
    if not domain:
        return None
    rdap = provider_payload_if_hit(enrichment, "rdap")
    whois = provider_payload_if_hit(enrichment, "whois")
    registrar_raw = rdap.get("registrar") or whois.get("registrar")
    registrar_name = str(registrar_raw).strip() if registrar_raw is not None else ""
    source = "rdap" if rdap.get("registrar") else ("whois" if whois.get("registrar") else "unknown")

    raw_nameservers = rdap.get("nameservers") or whois.get("nameservers") or []
    if not isinstance(raw_nameservers, list):
        raw_nameservers = []
    nameservers = sorted({
        _s(ns).strip().lower().rstrip(".") for ns in raw_nameservers if _s(ns).strip()
    })
    paid_alidns = bool(nameservers) and all(
        re.fullmatch(r"vip[1-8]\.alidns\.com", ns) for ns in nameservers  # leak-scan: allow 公共DNS产品识别规则
    )
    alidns = any(  # leak-scan: allow 公共DNS产品识别规则
        ns.endswith(".alidns.com") or ns == "alidns.com" for ns in nameservers  # leak-scan: allow 公共DNS产品识别规则
    )

    if paid_alidns:
        authoritative = {
            "provider": "Alibaba Cloud DNS",
            "product": "Public Authoritative DNS / Public Zone",
            "service_tier": "paid",
            "nameservers": nameservers,
            "jurisdiction": "unknown",
            "confidence": "high",
            "source": "rdap.nameservers+product_rule",
        }
    elif alidns:
        authoritative = {
            "provider": "Alibaba Cloud DNS",
            ...
            "jurisdiction": "unknown",
            "confidence": "medium",
            "source": "rdap.nameservers",
        }
    elif nameservers:
        authoritative = {
            "provider": "unknown", "product": "unknown", "service_tier": "unknown",
            "nameservers": nameservers, "jurisdiction": "unknown",
            "confidence": "low", "source": "rdap.nameservers",
        }
    else:
        authoritative = {
            "provider": "unknown", "product": "unknown", "service_tier": "unknown",
            "nameservers": [], "jurisdiction": "unknown",
            "confidence": "unknown", "source": "unknown",
        }

    if not registrar_name and not nameservers:
        return None
    return {
        "managed_zone": domain,
        "registrar": {
            "name": registrar_name or "unknown",
            "source": source,
            "confidence": "reported" if registrar_name else "unknown",
        },
        "authoritative_dns": authoritative,
        "zone_account": {"status": "unknown", "requires": "provider_legal_process"},
        "record_operator": {"status": "unknown", "requires": "console_or_api_audit_logs"},
        "limitations": [
