...
class EvidenceScope(Enum):
    ...
    ``case_evidence`` 是当前案件的直接检材/观测；``batch_reference`` 只作批量或跨案参考；
    ``legacy_unspecified`` 表示旧 schema 没记录作用域，不能静默当成当前案件直接证据。
    """
...
class Evidence:
    ...
    #: 对当前案件的证据资格。核心生产者默认产当前案件直接证据；旧报告由迁移器显式标 unknown。
    scope: EvidenceScope = EvidenceScope.CASE_EVIDENCE


def has_case_evidence(evidences: list[Evidence]) -> bool:
    """是否至少有一条当前案件直接证据；批次参考和旧版未说明均不具备独立结论资格。"""
    return any(ev.scope is EvidenceScope.CASE_EVIDENCE for ev in evidences)
...
#: 降档来源 → 它**取代**的来源。同一条观测（location + snippet + scope 全同）先以右侧来源
#: 写入、后来又以左侧来源写入时，旧的那条必须被撤销，而不是与新的并存。
#:
...
SUPERSEDED_EVIDENCE_SOURCES: dict[str, frozenset[str]] = {
    "runtime-derived": frozenset({"runtime-pcap"}),
}


def _evidence_identity(evidence: object) -> tuple[str, str, str]:
    """一条证据的**去来源**身份：同一次观测在不同归因结论下是同一条身份。"""
    def _get(field: str) -> str:
        if isinstance(evidence, dict):
            return str(evidence.get(field, ""))
        return str(getattr(evidence, field, ""))

    scope = _get("scope") or EvidenceScope.LEGACY_UNSPECIFIED.value
    if isinstance(scope, EvidenceScope):  # pragma: no cover - 属性访问返回枚举时
        scope = scope.value
    return (_get("location"), _get("snippet"), str(scope))


def revoke_superseded_evidence(existing: list, incoming: list) -> int:
    """按 :data:`SUPERSEDED_EVIDENCE_SOURCES` 就地撤销被取代的旧证据，返回撤销条数。

    ``existing`` 与 ``incoming`` 可以是 Evidence 对象或已序列化的 dict（两条回灌路径各用一种）。
    """
    superseded: dict[tuple[str, str, str], frozenset[str]] = {}
    for ev in incoming:
        source = str(ev.get("source", "")) if isinstance(ev, dict) else str(getattr(ev, "source", ""))
        replaced = SUPERSEDED_EVIDENCE_SOURCES.get(source)
        if replaced:
            superseded[_evidence_identity(ev)] = replaced
    if not superseded:
        return 0
    ...
    for ev in existing:
        source = str(ev.get("source", "")) if isinstance(ev, dict) else str(getattr(ev, "source", ""))
        if source in superseded.get(_evidence_identity(ev), frozenset()):
            continue
        kept.append(ev)
...
class Lead:
    ...
    def is_runtime_contact(self) -> bool:
        ...
        **标签**分档——标签本身的诚实性由 producer 侧保证：合成 / 派生路径须钉 ``runtime-derived`` 等非
        ...
        return any(
            ev.scope is EvidenceScope.CASE_EVIDENCE
            and str(getattr(ev, "source", "")) in OBSERVED_CONTACT_SOURCES
            for ev in self.source_refs
        )
...
DOWNGRADE_EVIDENCE_SCOPE: str = "evidence_scope"
...
def merge_runtime_into_lead_dict(
    ...
    *,
    ...
    追加进已有 ``source_refs``（去重 by (source, location, snippet, scope)），并据此重算
    ...
    # 归因结论把同一条观测判否时，先撤销它此前以 observed-contact 来源写下的那条——
    # 去重签名含 source，不撤销就会并存，而 is_runtime_contact 是存在量词、旧的仍算数。
    revoke_superseded_evidence(refs, incoming)

    def _scope_value(evidence: dict) -> str:
        raw = str(evidence.get("scope", EvidenceScope.LEGACY_UNSPECIFIED.value))
        try:
            return EvidenceScope(raw).value
        except ValueError:
    ...
    for ev in incoming:
        ...
        if scope == EvidenceScope.CASE_EVIDENCE.value:
            case_merged = True
    ...
    existing["is_runtime_seen"] = any(
        ...
        and str(ref.get("source", "")).startswith("runtime")
        for ref in refs
    )
    existing["is_runtime_contact"] = any(
        isinstance(ref, dict)
        and _scope_value(ref) == EvidenceScope.CASE_EVIDENCE.value
        and str(ref.get("source", "")) in OBSERVED_CONTACT_SOURCES
        for ref in refs
    )
    return case_merged, ledger_changed
...
#:        （在此之前它可自由增补：截至 v1.4.0 的全部发布 tag 写出的都是 ``"1.0"``，那时 1.1
#:        还没被任何消费方见过。）此后再加字段必须开新的次版本，并同步更新这段说明。
#:
#: ★**规范字段 vs 扩展区**：本版本号约束的是 fxapk 自己生成的那套字段（顶层 12 键，见
#:   ``tests/test_report_schema_contract.py`` 冻结的集合）。落盘的 report.json 顶层还可能出现
#:   **未知键**——那是载入时原样保留、写回时原样吐出的开放扩展区（见 ``report_io``），供外部
#:   工具挂自己的东西，不算核心 schema、不随本版本号变化。消费方据此区分：认识的键按本版本
#:   的契约读，不认识的键别当成 fxapk 的产出。
#:
#: 1.2 —— Evidence 增加 ``scope``，把当前案件直接证据与批量/跨案参考分开。旧 1.0/1.1
#:        缺失字段迁移为 ``legacy_unspecified``，安全方向不自动授予直接证据资格。
