...
def completed_from_records(
    ...
    for record in records:
        ...
        completed = {
            ...
            if isinstance(status, str) and status in LEDGER_COMPLETE_STATUSES
        }
        if completed:
...
def _status_map(endpoint: Endpoint) -> dict[str, str]:
    """从 ``endpoint.enrichment['source_status']`` 抽出 provider → status。"""
    raw = endpoint.enrichment.get("source_status")
