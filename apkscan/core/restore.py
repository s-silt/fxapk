...
# ``evidence_scope`` records a factual absence of direct current-case support,
# not an analyst policy suppression.  It can be resolved only by adding direct
# Evidence; a restore credential must never erase or bypass it.
NON_REVOCABLE_RESTORE_SOURCES = frozenset({"evidence_scope"})
...
def is_revocable_source(source: object) -> bool:
    """Whether a downgrade source may be lifted by an analyst credential."""
    normalized = _norm(source)
    return bool(normalized) and normalized not in NON_REVOCABLE_RESTORE_SOURCES
...
def restore_index(meta: Mapping[str, Any] | None) -> set[tuple[str, str, str]]:
    ...
    for item in raw:
        ...
        if any(not _norm(item.get(k)) for k in _REQUIRED):
            continue
        if not is_revocable_source(item.get("source")):
            continue
...
def record_restore(
    ...
    *,
    ...
    """
    if not is_revocable_source(source):
        raise ValueError(f"downgrade source is not manually revocable: {source}")
