...
class FourthPartyPaymentAnalyzer(BaseAnalyzer):
    ...
    meta_key_categories = {
        ...
        'fourth_party_payment_count': 'record',
        # 资源面覆盖缺口：让 count=0 能区分「确实没有」与「没扫全」。
        'fourth_party_payment_list_failed': 'coverage',
        'fourth_party_payment_read_failed': 'coverage',
        'fourth_party_payment_oversize_skipped': 'coverage',
        'fourth_party_payment_budget_exhausted': 'coverage',
    }
    ...
    def analyze(self, ctx: "AnalysisContext") -> AnalyzerResult:
        ...
        resource_coverage = self._scan_resources(ctx, endpoints, keywords, hits)
        ...
        for suffix, value in resource_coverage.items():
            if suffix == "list_failed":
                result.meta["fourth_party_payment_list_failed"] = value
            elif suffix == "read_failed":
                result.meta["fourth_party_payment_read_failed"] = value
            elif suffix == "oversize_skipped":
                result.meta["fourth_party_payment_oversize_skipped"] = value
            elif suffix == "budget_exhausted":
                result.meta["fourth_party_payment_budget_exhausted"] = value
