"""webview_jsbridge 分析器：WebView/JS-bridge 桥接面审查 → Finding + 桥接对象 Lead。
...
能力。把「H5 能调用哪些原生能力」「WebView 是否开了 file:// 跨源访问」作为桥接面线索产出。
...
class WebViewJsBridgeAnalyzer(BaseAnalyzer):
    ...
    meta_category_pending = frozenset({'webview_signals'})
            ...
            result.leads.append(
                Lead(
                    ...
                    notes=(
                        ...
                        + "：H5 前端可经此桥调用原生能力。结合 H5 调用面研判可被服务端下发内容驱动的原生桥接面。"
                    ),
                )
            )
