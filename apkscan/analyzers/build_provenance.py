...
平台产出的不同样本会复现同一构建根与批次标识。实测中绝大多数样本都含此类路径。

★ 头号风险（本模块分层设计的存在理由）：第三方 SDK / 开源项目的路径随源码或预编译
库**继承**进样本——Telegram 官方客户端作者的 macOS 主目录、RTC SDK 的 CI 构建账号、
Go 工具链、GitHub Actions 公共 CI 等在实测中大量出现（同一位开源作者的路径可以横跨
相当比例的样本）。把这些当成本样本作者的机器，会把无关的开源作者写进研判结论。
因此：
...
#: 标定依据：实测样本的人工核对——这些根来自公开 SDK / 开源项目 / 公共 CI，
...
_KNOWN_THIRD_PARTY_ROOTS: tuple[tuple[str, str], ...] = (
    ...
    # 实测验真补录：NDK sysroot/断言路径以 SDK 安装位开头（识别构建标识会得出
...
#:   这条前缀下挂着某个知名开源项目目录的人，还会凭"那是开源项目"把它再加回
#:   第三方名单——撤回理由必须和判据长在一起。
#: ★为什么撤回后不顺势判自建：原判据只是"与某类符号共现"，而同一批样本里的
#:   ``/users/dkaraush/`` 分布**完全相同**，却是真实的开源贡献者（其代码随上游分支一起
#:   被引入）。共现证明不了作者身份，"未知"比"第三方"和"自研"都诚实。
_WITHDRAWN_THIRD_PARTY_ROOTS: tuple[tuple[str, str], ...] = (
    (
        "/users/dhmac/",
        "曾列第三方 SDK 构建机，经实证撤回：命中样本很少，且原判据依赖的共现关系"
        "不足以支撑“第三方”这一分类；但共现同样不足以证明作者身份"
        "（同批样本的 /users/dkaraush/ 分布相同，却确为开源贡献者），"
        "故停在未知，待语料扩大后再判。",
    ),
)
