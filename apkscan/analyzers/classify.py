...
from apkscan.core.models import Evidence, EvidenceScope, Lead, LeadCategory
...
def _append_investigation_lead(report: "Report", best: _Score) -> None:
    ...
    **只追加新 Lead，绝不改动已有 Lead。**
    ...
    lead = Lead(
        ...
        notes=notes,
        advice="建议调证",
        # 分类结论是本案现有分析信号的聚合推导，不是批次参考。
        # 显式保留每条命中及其来源，使 report round-trip 后的 scope
        # 闸不会把一个正常 producer 产出的调证 Lead 当成 legacy 无证据。
        source_refs=[
            Evidence(
