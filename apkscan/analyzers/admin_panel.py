...
class AdminPanelAnalyzer(BaseAnalyzer):
    ...
    meta_key_categories = {
        'admin_panel_count': 'record',
        'dex_strings_truncated': 'coverage',
        # 资源面覆盖缺口：让 count=0 能区分「确实没有」与「没扫全」。
        'admin_panel_list_failed': 'coverage',
        'admin_panel_read_failed': 'coverage',
        'admin_panel_oversize_skipped': 'coverage',
        'admin_panel_budget_exhausted': 'coverage',
    }
    ...
    def analyze(self, ctx: "AnalysisContext") -> AnalyzerResult:
        ...
        for suffix, value in resource_coverage.items():
            if suffix == "list_failed":
                result.meta["admin_panel_list_failed"] = value
            elif suffix == "read_failed":
                result.meta["admin_panel_read_failed"] = value
            elif suffix == "oversize_skipped":
                result.meta["admin_panel_oversize_skipped"] = value
            elif suffix == "budget_exhausted":
                result.meta["admin_panel_budget_exhausted"] = value
