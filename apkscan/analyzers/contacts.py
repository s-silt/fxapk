...
class ContactsAnalyzer(BaseAnalyzer):
    ...
    def _maybe_getme(self, token: str, ctx: "AnalysisContext") -> str:
        ...
        默认不发：getMe 会用**接料人自己的 token** 主动触其 live bot，在 bot 持有者侧留下
        可观测痕迹、可能致 token 被撤换（对照 enrichers/asn.py 对明文 ip-api 的告警范式）。须**同时**满足
        ...
        if not _ctx_online(ctx):
            return (
                "未验证：默认离线未发 getMe（主动请求 api.telegram.org 会在对方侧留下"
                "可观测痕迹、可能致 token 被撤换）；如需在线核验请显式启用 online 能力。"
            )
        if _ctx_mode(ctx) != ANALYSIS_MODE_AUTHORIZED_ACTIVE:
            # ★ passive（默认）：getMe 会用**接料人自己的 token** 触其 live bot，属主动触达
            #   （在对方侧留下可观测痕迹），与被动分析定位冲突 → 不发，只留静态 token 线索。
            #   须 --mode authorized-active（操作者明确授权）才在线核验。
            return (
                "未验证：passive 模式未发 getMe（用接料人 token 主动触其 live bot 会在对方侧留下可观测痕迹）；"
