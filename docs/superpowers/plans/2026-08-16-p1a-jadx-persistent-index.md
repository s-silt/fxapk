...
- [ ] **Step 4: Implement the explicit status mapping.** Use `COMPLETE` + `CoverageStatus.COMPLETE` for a successful full rebuild; **for a verified cache hit, `OutcomeStatus` is `COMPLETE` but `CoverageStatus` is carried over from the manifest's recorded coverage（`COMPLETE` or `PARTIAL`）— a hit on a partially enumerated index must not be upgraded to complete coverage**; use `PARTIAL` + `PARTIAL` for usable positive hits with incomplete enumeration; use `FAILED` + `FAILED` for JADX/process failure with no usable positive result; use `FAILED` + `UNAVAILABLE` for `CacheUnavailable`; use `PARTIAL` + `TIMEOUT` for timeout with partial positive output and `FAILED` + `TIMEOUT` when no usable output exists. Cache miss/corruption/schema/tool drift are reason codes on the rebuild outcome, not negative findings. Do not emit any observation for empty results or absence.
