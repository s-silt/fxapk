...
# Codex 一键命令 / 快速上手（驱动 fxapk 做目标 APK 取证）
...
```
你驱动 fxapk（仓库 s-silt/fxapk）做目标 APK 取证。每次开工先自助更新：
...
【静态】fxapk analyze / auto 出 report.json（端点 / IP / 标识符 + 富化 + 归属分级 advice）。
...
按打法选：
  ...
  · frida 上不去 / 解不开（反 frida 秒退 / TLS pinning / MTProto 等自建协议 endpoint_total=0）→ 不碰 App 带外抓 pcap
    （PCAPdroid 免 root 导出 / 网关 tcpdump / Wireshark），照样拿接入节点 IP/SNI/DNS（穿透锚点，解不开也能溯源）：
...
【排噪音（重要，吃过亏）】线索的 advice 若是「待核」且 reason 含"疑似编码 / hex / base64 / 随机串 / 伪域名"——
  这是 base64/hex 串里夹了点被误当成域名，溯源不可回溯。**绝不拿去溯源、不反查、不回溯，人工核即可。**
  fxapk 已自动把这类降级为「待核」；你严格按 advice 只对「建议调证」目标动作，「待核 / 无需调证」一律不动手。

【回灌关联】探针线索 / pcap 线索都 `--into` 同一 report.json，合并后：
  fxapk graph 跨样本关联、fxapk letters 套打归属 / 溯源文书草稿。
...
- **新命令**：
  ...
  - `fxapk pcap-leads capture.pcap [--into report.json]` —— 带外 pcap → 接入节点 IP:port + SNI + DNS（纯标准库、零依赖；解不开也能溯源）
...
C. 保底出活 / 不想再跟反 frida 纠缠 → 带外 pcap（完全不注入，App 检测不到、也闪退不了）：
   ...
   fxapk pcap-leads capture.pcap --into report.json
   解不开也能溯源：带外拿到的接入节点 IP:port + SNI + DNS 就是穿透真源站的溯源锚点。
