...
| 优先级 | 调证对象 | 法律路径 | 现实可行性 |
|---|---|---|---|
| **P0** | **境内云/IDC 实例运营方 / 承租主体**（阿里云/腾讯云/华为云…，含 ESA/ENS/ECS） | 境内公安《反电诈法》/《网安法》协查函直接调取 | 高（境内直达；能解出**真实回源 IP + 实名账户 + 支付流水 + 全部轮换域 + 访问日志**——穿透隐源的钥匙） |
| P1 | 境内 IDC / 运营商（电信·联通·移动省网） | 境内公安协查 IP 承租记录 | 高（承租方常指向云厂商，与 P0 联动穿透） |
| P2 | 属地公安经侦 / 反诈中心 | 受害人立即报案 | 高（启动一切刑事调证的前提） |
