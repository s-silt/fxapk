<!-- 由 fxapk 方法调研工作流生成；个人安全研究/测试用途，发现涉诈线索可提供给相关部门依法处置。 -->

# 反分析涉诈 App —— frida 探针之外的抓包/取证方法目录

> 适用对象：办案人/Codex 驱动取证。前提：**全部操作须在合法授权下，对己方受控样本与隔离取证设备/测试网进行**；改写版样本不得外流；落地个人信息按办案合规留存处置。
...
分四层：**A 不碰 App 本体的旁路抓包 → B 静态改包去 pinning → C 非frida注入 / 对抗式frida → D 非网络取证**。每条给：工具 / 破哪种规避 / 拿到什么 / 调证价值 / 与 fxapk 怎么配 / 坑。
...
- 调证：现场 1 分钟部署，**按 App 精确归因**连了哪些节点；明文 URL 常含上报域名/落地参数。
...
- 调证：**最高穿透锚点价值**——接入 IP:port + SNI 直接调云厂商租户实名/定机房；对 HTTP 抓包 endpoint=0 的样本是唯一可靠数据源；可长期挂机抓节点轮换。
...
- 调证：比 ARP MITM 干净可控、比网关灵活（无需控路由器）；一条隧道集中一机全流量，**适合批量过样本一机一档**。
...
- 调证：只要同 Wi-Fi 就能牵流量拿锚点；dissector 实时显示连了哪些主机，适合快速判活。
...
- 调证：**JA4 指纹跨样本/跨节点聚类同一家黑产 SDK/打包器**，IP+SNI+JA4 三元组是调租户 + 把多个马甲 App 关联到同一团伙的硬锚点。
...
- 调证：**分钟级**拿可抓包样本，先批量捞明文 endpoint/IP/SNI 作初筛，硬骨头再上重武器。
...
- 调证：endpoint_total 从 0 变正的第一杠杆；改一次永久生效，产物哈希可复现固证。
...
- 调证：**最关键接入节点 IP:port + 明文协议，调证锚点最硬**。
...
- 调证：对**签名+代码双重完整性自检**的样本是唯一不改包的明文路径；适合长期蹲守同一样本。
...
- 调证：**极高**——加固涉诈样本核心逻辑与硬编码 C2/源站/密钥常藏壳内 DEX，脱壳即固证；去 pin 后抓明文形成"代码+流量"双锚点。
...
- 调证：明文 endpoint = 直接定位接入域名/接口/回传字段（账号/IMEI/银行卡）；明文里常含落地库/后台面板 URL。
...
- 调证：**APK 一字未改（哈希不变），动态观测与"原样样本"对应清楚，证据链最干净**；IP/SNI 可直接调云厂商租户。
...
- 调证：**绕过 80% 字符串级反frida**；即便只 hook 到 `connect()` 也能固证真实 IP:port+SNI。
...
- 调证：提供**稳定可复现的 root+frida 真机底座**，是批量调证流水线基础设施。
...
- 调证：针对最硬的 native 直发 + MTProto；即便协议私有，也能在 send/connect 处带外取 IP:port+SNI+时序。
...
- 调证：**无 root 严苛样本的少数可行路径**；明文/endpoint 可作锚点，但改包事实需在取证链说明（动态观测副本，固证仍以带外 IP/SNI 为准）。
...
- 调证：**极高**——既拿明文落地数据，又因是真实流量 pcap 完整保留接入节点元数据作固证；对 pinning+native直发+QUIC 这套"最难解"组合是少数能同时拿明文+元数据的路子。
...
- 调证：QUIC 的 SNI（Initial 包明文）+ 源站 IP:port + JA4 即便不解密也是强锚点；很多新涉诈样本特意走 QUIC 规避普通抓包，本法点亮盲区。
...
- 调证：**内容级铁证**——解开落地库=拿到联系人/交易/聊天；内存 key 让网络密文事后可解；dump 真 DEX 让静态分析拿到真业务逻辑。
...
- 调证：**最直接的内容级证据与穿透锚点**——落盘对象存储/接入域名补全"多云抗封堵"下发源清单；冷数据可重复勘验、链路完整、法庭采信度高。
...
- 调证：DNS 日志是独立、低成本、可长期留存的锚点，补全 pcap 看不全的域名维度（尤其对象存储多域名抗封堵）；`/proc/net` 抓 native 短连接补 pcap 漏抓；与 pcap 的 IP/SNI 三方交叉——"同时刻问了域名 X→连了 IP Y→SNI Z"形成强时序闭环。
...
## 三、"解不开也能办案"专章 —— 旁路 pcap 闭环到调证

> 核心命题：**当密文/协议都解不开（VMP/dex2c、签名+代码双完整性自检、native 直发、MTProto），靠传输层元数据 + 落地库即可单独立案闭环。**

**穿透锚点全集（密文不解也成立）：**
```
真实接入节点 dst IP:port  ──→ 向云厂商/IDC 调主机租户实名、备案、计费主体
TLS/QUIC 明文 SNI         ──→ 佐证"同一基础设施"、定位 vhost
JA3/JA4(C/S) 指纹         ──→ 跨样本/跨节点聚类同一黑产 SDK/打包器，关联多个马甲到同一团伙
...
5. **定人/穿透**：拿 IP:port 调云厂商租户实名 → 备案/计费主体 → 定人；JA4 三元组把多马甲并案；DNS+时序做节点轮换与活动画像。

**收口要点：** ECH 普及后 SNI 可能被加密 → 退到 **IP+JA4+DNS**；CDN/多云会让落地 IP 指向云厂商而非团伙自有机 → 必须**结合时序与租户调证**，别只看 IP 段；元数据含个人信息按办案合规留存。

---

## 四、最高 ROI 三招（探针之外只选 3 种先上）

对"刻意反分析的涉诈样本"，若只先上 3 种 frida 探针之外的方法，按"覆盖面 × 抗规避 × 部署成本"排序：
