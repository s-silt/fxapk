...
4. **深度归因**：对每个后端域名/IP `./scripts/fx-recon.sh <target>` 扇出 OSINT，据原始数据归因。
   - ★ **国内登记/承租主体**（阿里云/腾讯/华为/电信·联通·移动/IDC/有 ICP）= **最高调证优先级 P0**；即便被 fxapk 标「无需调证」也要从 `endpoints[].enrichment` 捞出来列为调证目标。境外服务器走**被动 IP 归属 + 穿透 CDN 定位真实源站**，不调证、不主动探测。
   - 区分边缘层 vs 真实源站（ESA/acw_tc/via:ens-cache=阿里云 ENS 边缘节点，非回源）。
   - **【排噪音·吃过亏】** 线索 `advice=待核` 且 reason 含"疑似编码 / hex / base64 / 随机串 / 伪域名"——这是 base64/hex 串里夹点被误当域名，调证**不可回溯**，**绝不拿去调证 / 回溯 / 反查，人工核即可**。fxapk 已自动把这类降待核；你严格只对 `advice=建议调证` 的目标动手，待核/无需调证一律不动。
...
红线：不编造 key-gated 结果；不把边缘节点当真实源站、不把**编码伪域名**当真域名回溯；探针**只读**（唯一出口 console.log、落盘仅设备临时目录、不操控）；被动归属查询仅对 `建议调证` 目标（不主动探测 / 不攻击第三方服务器）；可信度「高」只给实测/多源交叉；按现实可行性排调证优先级（境内可直查 > 境外司法协助），不按技术显眼度。
