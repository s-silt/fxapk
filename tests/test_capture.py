...
def test_target_attributed_survives_a_sibling_without_attribution(monkeypatch) -> None:  # noqa: ANN001
    """★ 一个承载端点确认属目标、另一个没归因时，``target_attributed`` 仍须为 True。

    ★这里的量词与降档判据**故意不同**，不可"顺手统一"：
        target_attributed： ∃ 已归因 carrier : is_target_app is True
        降档（is_denied）： 全部 carrier 已归因 ∧ 无一属目标
      前者问"有没有端点确认属目标"，后者问"能不能确证都不属目标"。
      同一组输入、两个问题、两个量词。

    实测教训：曾把本字段换成 ``verdict_for_carriers(...) is TARGET``（即降档那个量词），
    结果"一个端点确认属目标 + 另一个端点没归因"时它从 True 变成 False——
    把已确证的归属因为**别的**端点缺信息而抹掉了。当时来源标签的差分测试 32 组全绿，
    因为那验的是另一个输出，根本没覆盖本字段。
    """
    monkeypatch.setattr(pcap_ingest, "_ip_public", lambda value: value.startswith("100.64."))
    # 同一个域名端点由两个远端承载：一个确认属目标，另一个不在归因表里
    summary = pcap_ingest.PcapSummary(
        flows=[
            pcap_ingest.Flow(
                proto="tcp", src_ip="10.0.0.2", src_port=50010,
                dst_ip="100.64.7.1", dst_port=443,
                packets=2, payload_bytes=80, sni={"shared.example.test"},
            ),
            pcap_ingest.Flow(
                proto="tcp", src_ip="10.0.0.2", src_port=50011,
                dst_ip="100.64.7.2", dst_port=443,
                packets=2, payload_bytes=80, sni={"shared.example.test"},
            ),
        ]
    )
    ...
    capture._annotate_runtime_endpoints(
        ...
        summary,
        # 只有第一个远端有归因记录，第二个缺席
        {"tcp/100.64.7.1:443": {"is_target_app": True, "attribution": "confirmed"}},
    )

    domain = next(e for e in endpoints if e.value == "shared.example.test")
    assert domain.enrichment["runtime"]["target_attributed"] is True, (
        "★已确认属目标的端点，不该因为同名下另一个承载端点没归因而被抹掉"
    )
    # 同一输入下降档判据必须**不**降档（缺信息不得推出否定）——两个量词的分野就在这
    assert {ev.source for ev in domain.evidences} == {pcap_ingest.RUNTIME_PCAP_SOURCE}


def test_annotate_runtime_endpoints_downgrades_source_when_not_target(monkeypatch) -> None:  # noqa: ANN001
    """★ 归因判「不属于目标应用」时，端点证据的来源必须一并降档。

    capture 主路径的端点在归因**产出之前**就已建好并钉了 runtime-pcap，本函数是归因结果
    唯一能回写到它们身上的时机。此前只更新 enrichment，于是 target_attributed=false 的端点，
    其证据仍在 OBSERVED_CONTACT_SOURCES 里，下游照样渲染成「已抓到通信」。
    """
    monkeypatch.setattr(pcap_ingest, "_ip_public", lambda value: value == "100.64.9.20")
    summary = pcap_ingest.PcapSummary(
        flows=[
            pcap_ingest.Flow(
                proto="tcp", src_ip="10.0.0.2", src_port=50003,
                dst_ip="100.64.9.20", dst_port=443,
                packets=2, payload_bytes=80, sni={"other.example.test"},
            )
        ]
    )
    endpoints = pcap_ingest.to_runtime_endpoints(summary)
    assert {ev.source for ep in endpoints for ev in ep.evidences} == {
        pcap_ingest.RUNTIME_PCAP_SOURCE
    }, "前置：归因前应为 runtime-pcap"

    capture._annotate_runtime_endpoints(
        endpoints,
        summary,
        {"tcp/100.64.9.20:443": {"is_target_app": False, "attribution": "unattributed"}},
    )

    by_value = {endpoint.value: endpoint for endpoint in endpoints}
    for value in ("100.64.9.20", "other.example.test"):  # IP 与它的 SNI 域名都要降
        sources = {ev.source for ev in by_value[value].evidences}
        assert sources == {pcap_ingest.RUNTIME_UNATTRIBUTED_SOURCE}, f"{value} 未降档：{sources}"


def test_annotate_runtime_endpoints_keeps_source_without_attribution(monkeypatch) -> None:  # noqa: ANN001
    """反向：没有归因结论时不得降档——缺信息不写成否定。"""
    monkeypatch.setattr(pcap_ingest, "_ip_public", lambda value: value == "100.64.9.21")
    summary = pcap_ingest.PcapSummary(
        flows=[
            pcap_ingest.Flow(
                proto="tcp", src_ip="10.0.0.2", src_port=50004,
                dst_ip="100.64.9.21", dst_port=443, packets=2, payload_bytes=80,
            )
        ]
    )
    for attr in ({}, {"tcp/203.0.113.9:443": {"is_target_app": False}}):
        endpoints = pcap_ingest.to_runtime_endpoints(summary)
        capture._annotate_runtime_endpoints(endpoints, summary, attr)
        sources = {ev.source for ep in endpoints for ev in ep.evidences}
        assert sources == {pcap_ingest.RUNTIME_PCAP_SOURCE}, f"未归因不应降档：{attr} → {sources}"


def test_capture_annotates_endpoints_after_attribution() -> None:
    """★ 接线锁：抓包主路径必须是「先建端点 → 出归因 → 再回写」这个顺序。

    上面两条只调 _annotate_runtime_endpoints 本身，锁不住它在主路径里被摆在哪一步。
    若有人把回写挪到归因之前（或删掉调用），降档就永远不会发生，而那两条照样全绿。
    真机路径无法在单测里跑通，故按本仓既有做法对源码断言阶段顺序。
    """
    assert "_capture(" in inspect.getsource(capture.run), "主路径改道了，本用例锁错了函数"
    src = inspect.getsource(capture._capture)
    build = src.index("to_runtime_endpoints(")
    attribute = src.index("attribute_remote_endpoints(")
    annotate = src.index("_annotate_runtime_endpoints(")
    assert build < attribute < annotate, (
        f"阶段顺序被打乱：建端点@{build} / 归因@{attribute} / 回写@{annotate}"
    )
...
def test_parse_flows_extracts_server_ip(monkeypatch, tmp_path):  # noqa: ANN001
    ...
    flows = [
        _FakeFlowWithConn(
            _FakeRequest("https://gw.synthetic-c2a.vip/cfg", "gw.synthetic-c2a.vip", "https"),
    ...
    assert "gw.synthetic-c2a.vip" in by_value  # 域名也在
...
def test_is_noise_host_matching():
    ...
    assert capture._is_noise_host("gw.synthetic-c2a.vip", pats) is False  # 真涉诈域名不误杀
...
def test_parse_flows_filters_emulator_noise(monkeypatch, tmp_path):
    ...
    # 真涉诈域名 + 其实连 IP 保留
    assert "gw.synthetic-c2a.vip" in values
