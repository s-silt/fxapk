...
def test_ai_decrypt_lead_finding_and_meta_candidates(tmp_path) -> None:
    ...
    outcome = JadxAnalyzer()._scan_java(tmp_path)
    findings, candidates = outcome.findings, outcome.decrypt_candidates
...
def test_jadx_scan_java_emits_string_chain_finding(tmp_path) -> None:
    ...
    findings = JadxAnalyzer()._scan_java(tmp_path).findings
    chain = [f for f in findings if f.id == _FINDING_STRING_CHAIN]
    assert len(chain) == 1
...
# 跨静态可见样本的实测：修复后仍留存 230 条候选，其中 45 条在第三方库路径下、
