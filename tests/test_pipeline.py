...
class _ForgedNoRecordRdap(BaseEnricher):
    name = "rdap"
    applies_to = ["domain"]
    ...
    def enrich(self, ep: Endpoint) -> EnrichmentResult:
        return EnrichmentResult(
            ...
            ok=True,
            data={
                "_source_status": "no_record",
                "registrant": "FORGED-SUBJECT",
...
def test_online_enrichment_applied_and_failures_recorded(monkeypatch, fake_ctx):
    ...
    # 富化器异常被记录而非抛出；失败 payload 不得进入后续归因/闭环。
    assert "asn" not in ip_ep.enrichment
    assert ip_ep.enrichment["source_status"]["asn"] == {
        "status": "failed",
...
def test_no_record_provider_payload_cannot_raise_domain_lead(monkeypatch, fake_ctx):
    monkeypatch.setattr(pipeline, "discover_analyzers", lambda: [_GoodAnalyzer()])
    monkeypatch.setattr(pipeline, "discover_enrichers", lambda: [_ForgedNoRecordRdap()])
    monkeypatch.setattr(pipeline, "detect_capabilities", lambda online=True: {"online"})

    report = pipeline.run(fake_ctx, AnalysisConfig(online=True))

    endpoint = next(e for e in report.endpoints if e.kind == "domain")
    lead = next(l for l in report.leads if l.category == LeadCategory.DOMAIN)
    assert "rdap" not in endpoint.enrichment
    assert endpoint.enrichment["source_status"]["rdap"] == {"status": "no_record"}
    assert lead.subject != "FORGED-SUBJECT"
    assert lead.where_to_request != "域名注册商：FORGED-REGISTRAR"
    assert lead.confidence is Confidence.MEDIUM


def test_serialized_non_hit_provider_payloads_cannot_raise_endpoint_leads() -> None:
    from apkscan.core.leads import _domain_lead, _ip_lead

    evidence = Evidence(source="dex", location="classes.dex", snippet="api.example.com")
    domain = Endpoint(
        ...
        enrichment={
            "source_status": {
                ...
                for provider in ("icp", "rdap", "whois", "dns", "shodan", "certs", "virustotal")
            },
            "rdap": {"registrant": "FORGED-DOMAIN", "registrar": "FORGED-REGISTRAR"},
            "dns": {"hosting": [{"org": "FORGED-HOSTING"}]},
            "shodan": {"org": "FORGED-SHODAN", "ports": [443]},
            "certs": {"related_hostnames": ["forged.example"]},
            "virustotal": {"passive_dns": [{"value": "forged.example"}]},
        },
    )
    ip = Endpoint(
        value="198.51.100.10",
        ...
        enrichment={
            "source_status": {
                provider: {"status": "no_record"}
                for provider in ("asn", "shodan", "certs", "otx")
            },
            "asn": {"org": "FORGED-ASN", "asn": "AS64500"},
            "shodan": {"org": "FORGED-SHODAN", "ports": [443]},
            "certs": {"related_hostnames": ["forged.example"]},
            "otx": {"passive_dns": [{"value": "forged.example"}]},
        },
    )

    domain_lead = _domain_lead(domain)
    ip_lead = _ip_lead(ip)
    rendered = repr((domain_lead, ip_lead))
    assert "FORGED" not in rendered
    assert "forged.example" not in rendered
    assert domain_lead.subject is None
    assert ip_lead.subject is None
...
def test_app_tier_real_c2_not_downgraded():
    ...
    eps = [
        Endpoint(
            value="api.synthetic-c2a.vip",  # leak-scan: allow 合成 C2 夹具域名，基线既有值非案件真实值
            kind="domain",
            evidences=[Evidence(source="dex", location="classes.dex")],
            enrichment={"tier": "app"},
        ),
        Endpoint(
            value="pay.synthetic-c2b.com",  # leak-scan: allow 合成 C2 夹具域名，基线既有值非案件真实值
            kind="domain",
            evidences=[Evidence(source="dex", location="classes.dex")],
        ),  # 无 tier
    ]
        ...
        def analyze(self, ctx) -> AnalyzerResult:
            return AnalyzerResult(
                ...
                endpoints=[
                    ...
                    # 疑似 App 自有服务 → 建议调证
                    Endpoint(
                        value="pay.evil-app.com",  # leak-scan: allow 合成 C2 夹具域名，基线既有值非案件真实值
                        kind="domain",
                        evidences=[Evidence(source="dex", location="classes.dex")],
                    ),
                    # 公网 IP → 建议调证
...
def test_stage_attribution_clears_stale_views_after_non_hit_refresh() -> None:
    from types import SimpleNamespace
    ...
    endpoint = Endpoint(
        ...
        enrichment={
            "source_status": {
                "dns": {"status": "no_record"},
                "rdap": {"status": "failed"},
                "whois": {"status": "failed"},
            },
            "dns": {"hosting": [{"ip": "198.51.100.10", "asn": "AS64500"}]},
            "rdap": {"registrar": "FORGED-REGISTRAR"},
            "attribution": {"ips": [{"origin_network": {"name": "FORGED-AS64500"}}]},
            "domain_control": {"registrar": {"name": "FORGED-REGISTRAR"}},
        },
    )

    _stage_attribution(SimpleNamespace(endpoints=[endpoint]))
