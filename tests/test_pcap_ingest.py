...
def test_pcap_aggregates_remote_endpoint_across_five_tuples() -> None:
    ...
    out1 = _eth(_ipv4(_tcp_flags(b"A" * 100, 50000, 7689, 0x18), 6, "10.0.0.2", "47.98.207.14"), 0x0800)  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    synack = _eth(_ipv4(_tcp_flags(b"", 7689, 50000, 0x12), 6, "47.98.207.14", "10.0.0.2"), 0x0800)  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    in1 = _eth(_ipv4(_tcp_flags(b"B" * 70, 7689, 50000, 0x18), 6, "47.98.207.14", "10.0.0.2"), 0x0800)  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    summary = pcap_ingest.parse_pcap_bytes(_pcap([out1, synack, in1]))
    node = next(
        r for r in pcap_ingest.remote_endpoints(summary) if r.ip == "47.98.207.14" and r.port == 7689  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    )
    ...
    lead = next(
        l for l in pcap_ingest.to_report_leads(summary)
        if l.category == LeadCategory.IP and "47.98.207.14" in l.value  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    )
...
def test_fanzha_interception_node_excluded() -> None:
    ...
    fanzha, biz = "183.192.65.101", "43.230.113.177"  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
    ...
    assert "43.230.113.177" in ip_vals  # 业务接入节点正常保留  # leak-scan: allow pcap 远端/接入节点夹具，_ip_public 要判它是公网远端才会产出 runtime 端点
