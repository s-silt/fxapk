...
def test_load_runtime_endpoints_preserves_scope_and_fails_closed_when_missing(tmp_path) -> None:
    report_file = tmp_path / "runtime_report.json"
    _write_runtime_report(
        report_file,
        [
            ...
            },
            {
                ...
                "kind": "domain",
                "evidences": [{"source": "runtime", "location": "legacy.json"}],
            },
        ],
    )

    by_value = {ep.value: ep for ep in merge.load_runtime_endpoints(str(report_file))}

    assert by_value["case.example.com"].evidences[0].scope is EvidenceScope.CASE_EVIDENCE
    assert by_value["batch.example.com"].evidences[0].scope is EvidenceScope.BATCH_REFERENCE
    assert (
        by_value["legacy.example.com"].evidences[0].scope
        is EvidenceScope.LEGACY_UNSPECIFIED
    )
...
_C5B_KEY = "0123456789abcdef0123456789abcdef"
...
_C5B_PLAINTEXT = json.dumps(
    {
        ...
        "login": "/api/login",
        "webConfig": "https://gw.synthetic-c2a.vip/config",
        "inviteCode": "ABC123",
    },
    ensure_ascii=False,
)
...
def test_decrypt_runtime_messages_extracts_plaintext_endpoints(tmp_path) -> None:
    ...
    rr_path = _write_runtime_report_with_messages(
        tmp_path, [{"url": "https://api.synthetic-c2a.vip/post", "response_body": env}]
    )
    ...
    values = {ep.value for ep in report.endpoints}
    assert "https://gw.synthetic-c2a.vip/config" in values
    ...
    assert any("synthetic-c2a.vip" in l.value for l in domain_leads)
...
def test_decrypt_uses_runtime_recipe_over_static(tmp_path) -> None:
    ...
    rr_path = _write_rr(
        tmp_path,
        messages=[{"url": "https://api.synthetic-c2a.vip/post", "response_body": env}],
...
def test_merge_runtime_traces_anti_analysis_finding(tmp_path) -> None:
    ...
    assert aa[0].kind == "observation"  # 运行时实测行为 = 直接观测，非规则推导
