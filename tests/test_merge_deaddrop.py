...
def test_dead_drop_batch_and_legacy_runtime_refs_do_not_raise_confidence(tmp_path):
    rr = _write_runtime_report(
        tmp_path,
        [
            _config_msg(
                "https://rest.apizza.net/api/webConfig",  # leak-scan: allow 公开研究引用的 dead-drop 平台域，基线既有值
                '{"backend":"https://evil-c2.shop/in"}',  # leak-scan: allow 合成 dead-drop 夹具，基线既有值
            )
        ],
    )

    for scope in (EvidenceScope.BATCH_REFERENCE, EvidenceScope.LEGACY_UNSPECIFIED):
        reference_only = Endpoint(
            value="evil-c2.shop",  # leak-scan: allow 合成 dead-drop 夹具，基线既有值
            kind="domain",
            evidences=[
                Evidence(
                    source="runtime",
                    location="reference.json",
                    scope=scope,
                )
            ],
        )
        report = _make_report(endpoints=[reference_only])

        merge.resolve_dead_drop_c2(report, rr)

        lead = next(item for item in report.leads if item.value == "evil-c2.shop")  # leak-scan: allow 合成 dead-drop 夹具，基线既有值
        assert lead.confidence == Confidence.MEDIUM, scope
