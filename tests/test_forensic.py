...
def test_classify_foreign_country_is_foreign() -> None:
    ...
    # 港澳台按境外 / 难直接调处理。
    assert (
        forensic.classify_jurisdiction("evilbackend.example.com", asn={"country": "Hong Kong"})
        == forensic.JURIS_FOREIGN
    )
...
def test_domain_lead_domestic_gets_investigation_path() -> None:
    ep = _ep("evilbackend.example.com", icp={"license_no": "京ICP备1号", "subject": "某公司"})
    lead = _domain_lead(ep, online=True)
    assert lead.advice == "建议调证"
...
def test_ip_lead_foreign_gets_forensic_path() -> None:
    ...
    ep = _ep("45.79.10.20", kind="ip", asn={"country": "United States", "org": "Example LLC"})  # leak-scan: allow 取证链夹具，须是公网 IP 才得到 ADVICE_INVESTIGATE
