...
@pytest.mark.parametrize(("domain", "provider", "bucket"), [
    ...
    ("impyq.gz.bcebos.com", "百度智能云 BOS", "impyq"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("ba63692667a911cc.gz.bcebos.com", "百度智能云 BOS", "ba63692667a911cc"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("9af1d19312f037db.gz.bcebos.com", "百度智能云 BOS", "9af1d19312f037db"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("840a5a8538b6f646.gz.bcebos.com", "百度智能云 BOS", "840a5a8538b6f646"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("aa112211.oss-cn-hangzhou.aliyuncs.com", "阿里云 OSS", "aa112211"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("f4e0094dd5652b2e.oss-accelerate.aliyuncs.com", "阿里云 OSS", "f4e0094dd5652b2e"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("ac265ebfeccb6c6d.oss-accelerate.aliyuncs.com", "阿里云 OSS", "ac265ebfeccb6c6d"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("sk3k22wkl4sh.s3.il-central-1.amazonaws.com", "AWS S3", "sk3k22wkl4sh"),  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    ("vayps1310hrz-1441221961.cos.accelerate.myqcloud.com", "腾讯云 COS",  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
     "vayps1310hrz-1441221961"),
])
...
def test_bucket_check_runs_before_the_blanket_vendor_exemption() -> None:
    ...
    """
    advice, reason = infra.classify_domain("impyq.gz.bcebos.com")  # leak-scan: allow 判据夹具：桶/端点形态本身就是被测对象，换占位域即失去被测形态
    assert advice == _INV
