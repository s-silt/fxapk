...
def test_jadx_execution_receipt_reaches_visibility(fake_ctx, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """★B2 消费端接线（真入口）：pipeline.run → _stage_run_analyzers 写 meta.analyzer_receipts
    （jadx 落 long lane、异常定 analyzer_error）→ _stage_visibility 消费 → java 面定 failed 并阻断
    Java 穷尽性主张。突变验证：删掉 _stage_run_analyzers 里 analyzer_receipts 那行赋值，
    java 只能落回 unknown、本测必红。"""
    ...
    class _BoomJadx:
        ...
        def analyze(self, ctx):  # type: ignore[no-untyped-def]
            raise RuntimeError("jadx boom")

    _stub_discovery(monkeypatch, analyzers=[_BoomJadx()])
    report = pipeline.run(fake_ctx, AnalysisConfig(online=False))

    assert report.meta["analyzer_receipts"]["jadx"] == {
        "lane": "long", "execution": "analyzer_error",
    }
    vis = report.meta["visibility"]
    assert vis["sources"]["java"]["visibility"] == "failed"
    assert "static_endpoint_exhaustive" in vis["blocked_claims"]
    assert "no_hardcoded_credential" in vis["blocked_claims"]
