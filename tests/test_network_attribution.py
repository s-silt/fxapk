...
from apkscan.core.models import Endpoint, Evidence, EvidenceScope
...
def _build_or_fail(*endpoints, phase="analyze") -> dict[str, Any]:
    blob = _build(*endpoints, phase=phase)
...
def test_non_hit_provider_payloads_cannot_form_network_attribution_evidence(
    status: str,
) -> None:
    source_status = {
        provider: {"status": status}
        for provider in ("dns", "certs", "shodan", "asn")
    }
    domain = _ep(
        "a.example.com",
        "domain",
        {
            ...
            "certs": {"related_hostnames": ["cert.example.com"]},
            "shodan": {"hostnames": ["scan.example.com"]},
        },
    )
    ip = _ep(
        ...
        {
            "source_status": source_status,
            "asn": {"asn": "AS64500", "country": "CN"},
            "shodan": {"asn": "AS64501"},
        },
    )

    assert _build(domain, ip) is None


def test_canonical_hit_provider_payloads_form_network_attribution_evidence() -> None:
    domain = _ep(
        "a.example.com",
        "domain",
        {
    ...
    edges = _edges(_build_or_fail(domain))

    assert ("resolves_to", "a.example.com", "198.51.100.10") in edges
    assert ("intel_related", "a.example.com", "cert.example.com") in edges


def test_legacy_payload_without_source_status_remains_readable() -> None:
    endpoint = _ep(
        "a.example.com",
        "domain",
        {"dns": {"ips": ["198.51.100.10"]}},
    ...
    assert ("resolves_to", "a.example.com", "198.51.100.10") in _edges(_build_or_fail(endpoint))


def test_present_source_status_map_missing_provider_fails_closed() -> None:
    endpoint = _ep(
        "a.example.com",
        "domain",
        {
    ...
    assert _build(endpoint) is None
...
def test_domestic_and_direct_connection_from_facts() -> None:
    blob = _build_or_fail(_cn_ip_endpoint())
...
def test_no_static_report_yields_an_eligible_role() -> None:
    ...
    blob = _build_or_fail(_cn_ip_endpoint(), _ep("x.example.com", "domain", {
        "dns": {"ips": ["198.51.100.8"]},
        "attribution": {"ips": [{"ip": "198.51.100.8", "country": "US",
                                 "origin_network": {"asn": 13335, "category": "cdn"},
...
def _runtime_dom(
    ...
    return _ep(
        value,
        "domain",
        {"runtime": {"sni": list(sni or [value]), "remote_endpoints": list(ips)}},
        evidences=[Evidence(source="runtime-pcap", location="pcap", scope=scope)],
    )
...
def test_batch_and_legacy_runtime_domain_pairing_cannot_license_tls_sni() -> None:
    for scope in (EvidenceScope.BATCH_REFERENCE, EvidenceScope.LEGACY_UNSPECIFIED):
        assert _build(_runtime_dom(scope=scope)) is None, scope


def test_runtime_ip_becomes_network_flow_apk_contacted_and_mints_apk_node() -> None:
    blob = _build_or_fail(_runtime_ip("203.0.113.7"))
    edges = _edges(blob)
...
def test_runtime_remote_endpoint_ipv6_port_is_stripped_on_last_colon() -> None:
    blob = _build_or_fail(_runtime_dom(ips=["2001:db8::1:443"]))
    assert ("served_at", "api.example.com", "2001:db8::1") in _edges(blob)


def test_tls_sni_dedups_one_ip_seen_on_multiple_ports() -> None:
    blob = _build_or_fail(_runtime_dom(ips=["203.0.113.7:443", "203.0.113.7:8443"]))
...
def test_network_flow_fires_for_mitm_only_ip_without_runtime_dict() -> None:
    ...
    blob = _build_or_fail(_runtime_ip("198.51.100.5", source="runtime", enrichment={}))
...
def test_no_apk_node_without_a_runtime_contacted_edge() -> None:
    ...
    blob = _build_or_fail(_ep("a.example.com", "domain", {"dns": {"ips": ["1.2.3.4"]}}))
    assert not any(n["type"] == "APK" for n in blob["graph"]["nodes"])


def test_runtime_edge_confidence_is_constant_0_95() -> None:
    blob = _build_or_fail(_runtime_dom(ips=["203.0.113.7:443"]), _runtime_ip("203.0.113.7"))
...
def test_only_peer_observing_runtime_sources_license_network_flow() -> None:
    ...
    for good_source in ("runtime", "runtime-pcap"):
        blob = _build_or_fail(_runtime_ip("203.0.113.99", source=good_source))
        assert any(e["type"] == "network_flow" for e in blob["evidence"]), good_source


def test_batch_and_legacy_runtime_evidence_cannot_license_network_flow() -> None:
    for scope in (EvidenceScope.BATCH_REFERENCE, EvidenceScope.LEGACY_UNSPECIFIED):
        endpoint = _ep(
            "203.0.113.99",
            "ip",
        ...
        assert _build(endpoint) is None, scope
    ...
    assert any(e["type"] == "network_flow" for e in _build_or_fail(endpoint)["evidence"])
...
def test_batch_and_legacy_runtime_evidence_cannot_license_direct_connection() -> None:
    def ip_ep(scope: EvidenceScope) -> Endpoint:
        return _ep(
            "203.0.113.9",
            "ip",
            {
    ...
    for scope in (EvidenceScope.BATCH_REFERENCE, EvidenceScope.LEGACY_UNSPECIFIED):
        assert "direct_connection" not in _matched_signals(_build(ip_ep(scope)), "203.0.113.9")
    assert "direct_connection" in _matched_signals(
        _build(ip_ep(EvidenceScope.CASE_EVIDENCE)), "203.0.113.9"
    )
