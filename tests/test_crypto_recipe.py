...
- 本地 示例应用 真值验证（skipif 保护，无样本环境不挂）：key==55f0… / AES / iv 含 md5。
- 无 CryptoJS → 不产配方。
...
_HUACAI_APK = Path(__file__).resolve().parent.parent / "ybku" / "示例应用-S0jZhGw4enKpBumE.apk"

# 合成 JS：CryptoJS AES-CFB/Pkcs7 + 硬编码 key（utf8）+ iv=MD5(key+ts).substring(0,16)
# + 请求信封 {timestamp,data} —— 复刻 示例应用 形态（值是合成的，不是真 key）。
_SYNTHETIC_JS = """
var cu = CryptoJS;
...
# ---------------------------------------------------------------------------
# 本地 示例应用 真值验证（skipif 保护）
# ---------------------------------------------------------------------------


@pytest.mark.skipif(not _HUACAI_APK.exists(), reason="本地无 示例应用 样本，跳过真值验证")
def test_示例应用_groundtruth_key_extracted() -> None:
