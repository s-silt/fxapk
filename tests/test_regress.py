...
def _report(sha: str, version: str, *, status="complete", hardened=False,
    ...
    closure_value = "closure.example.test"
    closure_meta = (
        {
            "status": closure,
            **(
                {"targets": [{"kind": "domain", "value": closure_value}]}
                if closure == "complete"
                else {}
            ),
        }
    ...
    return {
        ...
        "meta": {
            ...
            **({"closure": closure_meta} if closure_meta is not None else {}),
            **({"visibility": visibility} if visibility is not None else {}),
        },
        "findings": [{"id": f} for f in findings],
        "leads": [{
            "category": "DOMAIN", "value": f"d{i}.test", "advice": a,
            "source_refs": [{
                "source": "dex", "location": "classes.dex", "scope": "case_evidence",
            }],
        }
                  for i, a in enumerate(leads)],
        "endpoints": ([{
            "kind": "domain", "value": closure_value,
            "evidences": [{
                "source": "runtime-pcap", "location": "capture.pcap",
                "scope": "case_evidence",
            }],
        }] if closure == "complete" else []),
    }
...
def test_closure_status_revalidates_complete_against_direct_case_evidence():
    supported = _report("s1", "1.0.0", closure="complete")
    unsupported = _report("s1", "1.0.0", closure="complete")
    unsupported["endpoints"] = []
    empty = _report("s1", "1.0.0", closure="complete")
    empty["meta"]["closure"]["targets"] = []

    assert regress._closure_status(supported) == "complete"
    assert regress._closure_status(unsupported) == "partial"
    assert regress._closure_status(empty) == "failed"
...
def test_available_versions_follows_insertion_order(tmp_path):
    """★修订版按 catalog 权威入库顺序返回，不是字典序。

    字典序里 "1.10.0" 排在 "1.9.0" 前面，取最后一个当「最新版」会拿到 1.9——方向直接反了。
    新入库记录由 catalog 锁内分配 ingest_sequence；manifest 行序与文件 mtime 都不是顺序事实。
    """
...
def test_available_versions_uses_catalog_sequence_after_reindex(tmp_path):
    root = _seed(tmp_path, [
        _report("s1", "2.0.0"),
        _report("s1", "1.0.0"),
    ])

    before, before_authoritative = regress.available_versions_with_order(
        corpus.load_materialized_manifest(root)
    )
    rebuilt = corpus.reindex(root)
    after, after_authoritative = regress.available_versions_with_order(rebuilt)

    assert before_authoritative is True
    assert after_authoritative is True
    assert before == after == ["2.0.0@dd", "1.0.0@dd"]
    assert sorted(entry["ingest_sequence"] for entry in rebuilt) == [1, 2]


def test_cli_regress_legacy_order_requires_both_explicit_revisions(tmp_path):
    ...
    loaded = corpus.load_materialized_manifest(root)
    revisions, authoritative = regress.available_versions_with_order(loaded)
    assert revisions == ["2.0.0@dd", "1.0.0@dd"]
    assert authoritative is False

    default = CliRunner().invoke(
        cli.app,
        ["corpus", "regress", "--corpus", str(root), "--json"],
    )
    explicit = CliRunner().invoke(
        cli.app,
        [
            "corpus", "regress", "--corpus", str(root), "--json",
            "--from", "2.0.0@dd", "--to", "1.0.0@dd",
        ],
    )

    assert default.exit_code == 2
    assert "不能据此猜测" in default.stderr
...
def test_nonfinite_report_is_unreadable_not_a_false_regression(tmp_path, token):
    root = _seed(tmp_path, [
        _report("s1", "1.0.0", leads=["建议调证"] * 3, closure="complete"),
        _report("s1", "1.1.0", leads=["建议调证"] * 3, closure="complete"),
    ])
    [newer] = [
        entry
        for entry in corpus.load_materialized_manifest(root)
        if entry["tool_version"] == "1.1.0"
    ]
    path = root / str(newer["report_path"])
    raw = path.read_text(encoding="utf-8")
    path.write_text(raw[:-1] + f', "nonfinite": {token}}}', encoding="utf-8")

    diffs, summary = regress.load_and_diff(root, "1.0.0@dd", "1.1.0@dd")

    assert diffs[0].advice_from == {"建议调证": 3}
    assert diffs[0].advice_to is None
    assert summary["advice_unreadable"] == 1
    assert summary["closure_downgraded"] == 0
    assert not any("→" in note for note in diffs[0].notes)


@pytest.mark.parametrize("payload", [[], ["not", "an", "object"], 42, "text"])
def test_load_report_rejects_non_object_json_without_raising(tmp_path, payload):
    root = tmp_path / "corpus"
    path = root / "reports" / "legacy" / "bad.report.json"
    path.parent.mkdir(parents=True)
    path.write_text(json.dumps(payload), encoding="utf-8")
    entry = {
        "sample_sha256": "legacy",
        "tool_version": "1.0.0",
        "ruleset_digest": "dd",
        "report_path": path.relative_to(root).as_posix(),
    }

    assert regress._load_report(root, entry) is None


def test_tampered_report_does_not_create_false_regression_conclusions(tmp_path):
    root = _seed(tmp_path, [
        _report("s1", "1.0.0", leads=["建议调证"] * 2, closure="complete"),
        _report("s1", "1.1.0", leads=["建议调证"] * 2, closure="complete"),
    ])
    [newer] = [
        entry
        for entry in corpus.load_materialized_manifest(root)
    ...
    forged["leads"] = []
    forged["meta"]["closure"] = {"status": "failed"}
    ...
    assert summary["advice_unreadable"] == 1
    assert summary["closure_downgraded"] == 0
    assert not any("建议调证 2 → 0" in note for note in diffs[0].notes)
...
def test_advice_counts_scope_projects_batch_and_legacy_to_review():
    report = _report("s1", "1.0.0")
    report["leads"] = [
        {
            "category": "DOMAIN", "value": "batch.example.test", "advice": "建议调证",
            "source_refs": [{
                "source": "batch", "location": "batch.csv", "scope": "batch_reference"
            }],
        },
        {
    ...
    assert regress.advice_counts(report) == {"待核": 2}


def test_advice_counts_accepts_matching_direct_endpoint_evidence():
    report = _report("s1", "1.0.0")
    report["leads"] = [
        {
    ...
    report["endpoints"] = [
        {
            "kind": "domain", "value": "direct.example.test",
            "evidences": [{
                "source": "runtime-pcap", "location": "capture.pcap",
                "scope": "case_evidence",
            }],
    ...
    assert regress.advice_counts(report) == {"建议调证": 1}
...
def test_regress_preserves_all_case_bindings_in_core_json_and_text(tmp_path):
    from typer.testing import CliRunner
    ...
    for report in reports:
        ...
        corpus.add_report(root, report, raw, case_id="case-b")

    diffs, _summary = regress.load_and_diff(root, "1.0.0@dd", "1.1.0@dd")
    assert diffs[0].case_ids == ["case-a", "case-b"]
    assert diffs[0].case_id is None

    json_result = CliRunner().invoke(
        cli.app, ["corpus", "regress", "--corpus", str(root), "--json"]
    )
    assert json_result.exit_code == 0, json_result.output
    payload = json.loads(json_result.stdout)
    assert payload["diffs"][0]["case_ids"] == ["case-a", "case-b"]
    assert payload["diffs"][0]["case_id"] is None

    text_result = CliRunner().invoke(
        cli.app, ["corpus", "regress", "--corpus", str(root)]
    )
    assert text_result.exit_code == 0, text_result.output
    assert "案=case-a、case-b" in text_result.stdout


@pytest.mark.parametrize("report_path", ["../outside.json", "C:/outside.json"])
def test_regress_never_reads_report_path_outside_corpus(tmp_path, report_path):
    outside = tmp_path / "outside.json"
    outside.write_text(json.dumps(_report("s1", "1.0.0")), encoding="utf-8")
    root = tmp_path / "corpus"
    root.mkdir()

    assert regress._load_report(root, {"report_path": report_path}) is None


def test_regress_never_follows_symlink_outside_corpus(tmp_path):
    outside = tmp_path / "outside.json"
    outside.write_text(json.dumps(_report("s1", "1.0.0")), encoding="utf-8")
    root = tmp_path / "corpus"
    root.mkdir()
    ...
    assert regress._load_report(root, {"report_path": "linked.json"}) is None
