...
def test_generates_expected_md5_subdomain() -> None:
    days = [date(2026, 7, 23)]
    out = md5_date_subdomains("PREFIX-000-", "example-synthetic.test", days, path="/x.txt")
    assert len(out) == 1
    c = out[0]
    assert c["subdomain"] == f"{_md5('PREFIX-000-20260723')}.example-synthetic.test"
...
def test_cli_config_channel() -> None:
    ...
    r = CliRunner().invoke(cli.app, [
        "config-channel", "--prefix", "PREFIX-000-", "--domain", "example-synthetic.test",
    ...
    assert payload["candidates"][0]["subdomain"] == f"{_md5('PREFIX-000-20260723')}.example-synthetic.test"
