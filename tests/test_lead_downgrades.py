...
def test_downgrades_survive_report_roundtrip() -> None:
    ...
    lead = _lead(source_refs=[Evidence(source="dex", location="classes.dex")])
    apply_downgrade(lead, DOWNGRADE_REPACK_IDENTITY, "样本判为正版重打包件")
    apply_downgrade(lead, DOWNGRADE_SNI_MASQUERADE, "只在非标端口作 SNI 出现")
...
def test_new_fields_survive_real_disk_roundtrip(tmp_path) -> None:
    ...
    fresh = _lead(source_refs=[Evidence(source="dex", location="classes.dex")])
...
def test_legacy_report_without_scope_is_quarantined() -> None:
    """★旧报告仍可读，但不得把未声明作用域的材料伪装成当前案件直接证据。"""
    from apkscan.core import report_io
    ...
    assert set(revived.downgrades) == {DOWNGRADE_EVIDENCE_SCOPE}
    assert "当前案件" in revived.downgrades[DOWNGRADE_EVIDENCE_SCOPE]
