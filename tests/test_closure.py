...
class _ClosureLayer(TypedDict):
    status: str
    evidence: dict[str, object]
    evidence_fields: list[str]


class _TargetSelection(TypedDict):
    candidate_total: int
    selected: int
    limit: int
    truncated: int
...
class _ClosureCheck(TypedDict):
    id: str
    status: str
    reason: object


class _TargetClosure(TypedDict):
    status: str
    layers: dict[str, _ClosureLayer]
    origin: dict[str, object]
    gaps: list[str]
    source_status: dict[str, dict[str, object]]
    resolved_ips: list[str]
    resolved_ip_selection: _TargetSelection
    runtime: dict[str, object]


class _ClosureResult(TypedDict):
    status: str
    gaps: list[str]
    checks: list[_ClosureCheck]
    targets: list[_TargetClosure]
    target_selection: _TargetSelection


def _assemble_target(endpoint: Endpoint) -> _TargetClosure:
    return cast(_TargetClosure, assemble_target_closure(endpoint))
...
def _close(
    ...
    *,
    ...
    return cast(_ClosureResult, close_report(report, config, enrichers=enrichers))
...
def _evaluate(
    ...
    *,
    ...
    target_selection: _TargetSelection | None = None,
) -> _ClosureResult:
    return cast(
        _ClosureResult,
        evaluate_closure(
...
def test_closure_normalizes_legacy_source_status_strings() -> None:
    endpoint = _complete_endpoint()
    endpoint.enrichment["source_status"] = {
        "rdap": "hit",
        "fofa": "quota_insufficient",
        "hunter": {"status": "not_applicable"},
    }

    target = closure_module._single_target_closure(endpoint)

    assert target["source_status"] == {
        "fofa": {"status": "failed", "error_type": "quota_insufficient"},
        "hunter": {"status": "skipped", "reason": "not_applicable"},
        "rdap": {"status": "hit"},
    }


def test_close_report_canonicalizes_unselected_and_nested_source_status() -> None:
    selected = _complete_ip_endpoint("198.51.100.10")
    unselected = _complete_ip_endpoint("198.51.100.11")
    selected.enrichment["source_status"]["rdap"] = "hit"
    unselected.enrichment["source_status"] = {"rdap": "no_record"}
    unselected.enrichment["resolved_ip_enrichment"] = {
        "198.51.100.12": {"source_status": {"shodan": "quota_insufficient"}}
    }
    report = _report(selected, unselected)

    close_report(
        report,
        ClosureConfig(online=False, max_targets=1, require_dynamic=False),
        enrichers=[],
    )

    assert selected.enrichment["source_status"]["rdap"] == {"status": "hit"}
    assert unselected.enrichment["source_status"] == {
        "rdap": {"status": "no_record"}
    }
    assert unselected.enrichment["resolved_ip_enrichment"]["198.51.100.12"][
        "source_status"
...
def test_assemble_target_closure_builds_all_investigation_layers() -> None:
    target = _assemble_target(_complete_endpoint())
...
def test_fofa_product_evidence_can_complete_hosting_without_shodan() -> None:
    ...
    endpoint.enrichment["source_status"]["fofa"] = {"status": "hit"}
...
def test_cdn_without_origin_cannot_be_complete() -> None:
    ...
    target = _assemble_target(report.endpoints[0])
    target["origin"] = {"required": True, "status": "missing"}

    closure = _evaluate(report, [target], require_dynamic=False)
...
def test_static_critical_failure_makes_closure_failed() -> None:
    ...
    closure = _evaluate(
        report,
        [_assemble_target(report.endpoints[0])],
...
def test_static_only_targets_blocked_when_dex_invisible() -> None:
    ...
    closure = _evaluate(
        report, [_assemble_target(report.endpoints[0])], require_dynamic=False
    )
...
def test_runtime_attributed_targets_not_downgraded_by_static_blindness() -> None:
    ...
    assert "runtime-attributed" in str(vis_check["reason"])
...
def test_visible_report_passes_visibility_check() -> None:
    ...
    # java（JADX 反编译面）同理——receipt.complete=True 是该面完整覆盖的唯一凭据。
    report.meta["visibility"] = visibility.assess(
        {"meta": {
            "dex_available": True, "resource_files_scanned": 12,
            "jadx_receipt": {"status": "ok", "complete": True, "reason_codes": []},
        }}
    )
...
def test_monkeypatch_target_differs_by_caller() -> None:
    ...
    def _global_names(code: object) -> set[str]:
        ...
        """
        names = {str(name) for name in getattr(code, "co_names", ())}
...
def test_failed_provider_payloads_cannot_complete_closure_layers() -> None:
    endpoint = _endpoint(
        "198.51.100.10",
        runtime=True,
        target=True,
        ...
        enrichment={
            "source_status": {
                "ip_rdap": {"status": "failed", "error_type": "timeout"},
                "ripestat_bgp": {"status": "failed", "error_type": "timeout"},
    ...
    closure = _assemble_target(endpoint)

    assert closure["layers"]["resource_registration"]["status"] == CLOSURE_FAILED
    assert closure["layers"]["bgp_announcement"]["status"] == CLOSURE_FAILED
    assert closure["layers"]["hosting_delivery"]["status"] == CLOSURE_FAILED
    assert closure["layers"]["request_target"]["status"] == CLOSURE_FAILED


def test_registration_layer_fails_closed_when_status_map_omits_provider() -> None:
    from apkscan.core.closure.layers import _registration_layer

    layer = _registration_layer({
...
def test_closure_attribution_refresh_clears_stale_view_after_non_hit() -> None:
    endpoint = _endpoint(
        "198.51.100.10",
        enrichment={
            "source_status": {
            ...
            "asn": {"asn": "AS64500", "org": "FORGED-ASN"},
            "ip_rdap": {"netname": "FORGED-NET"},
            "attribution": {"ips": [{"origin_network": {"name": "FORGED-AS64500"}}]},
            "domain_control": {"registrar": {"name": "FORGED-REGISTRAR"}},
        },
    )
...
def test_close_report_top_level_attribution_absorbs_resolved_evidence(monkeypatch) -> None:  # noqa: ANN001
    ...
    _close(report, ClosureConfig(online=True, require_dynamic=False), enrichers=[dns, *ip_enrichers])
...
def test_domain_resolution_excludes_known_intercept_address(monkeypatch) -> None:  # noqa: ANN001
    ...
    domain = _endpoint(
        ...
        enrichment={
            "source_status": {"dns": {"status": "hit"}},
            "dns": {"ips": ["198.51.100.10", "198.51.100.20"]},
        },
    )
    report = _report(domain)
...
def test_resolved_ips_requires_dns_hit_when_source_status_exists(monkeypatch) -> None:
    monkeypatch.setattr(closure_sources, "_normalized_public_ip", lambda value: str(value).strip())
    stale_dns = {"ips": ["198.51.100.77"]}
    ...
    for status in ("failed", "no_record", "skipped", "disabled"):
        ...
        assert closure_sources._resolved_ips(endpoint) == []
    ...
    assert closure_sources._resolved_ips(missing) == []


def test_resolved_ips_accepts_dns_hit_and_true_legacy(monkeypatch) -> None:
    monkeypatch.setattr(closure_sources, "_normalized_public_ip", lambda value: str(value).strip())
    payload = {"dns": {"ips": ["198.51.100.77"]}}
    hit = _endpoint(
        "probe.example.test",
        kind="domain",
        enrichment={"source_status": {"dns": {"status": "hit"}}, **payload},
    )
    legacy = _endpoint("probe.example.test", kind="domain", enrichment=payload)

    assert closure_sources._resolved_ips(hit) == ["198.51.100.77"]
    assert closure_sources._resolved_ips(legacy) == ["198.51.100.77"]


def test_resolved_ips_rejects_canonicalized_legacy_private_failure(monkeypatch) -> None:
    from apkscan.core.source_status import canonicalize_source_status_tree

    monkeypatch.setattr(closure_sources, "_normalized_public_ip", lambda value: str(value).strip())
    enrichment = {
        "dns": {"_status": "failed", "ips": ["198.51.100.77"]},
    }
    ...
    assert enrichment["source_status"]["dns"]["status"] == "failed"
    assert closure_sources._resolved_ips(endpoint) == []
