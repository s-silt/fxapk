...
from apkscan.core.models import AnalysisConfig, Endpoint, Evidence, EvidenceScope
...
def test_batch_and_legacy_evidence_cannot_raise_asset_score() -> None:
    for scope in (EvidenceScope.BATCH_REFERENCE, EvidenceScope.LEGACY_UNSPECIFIED):
        endpoint = Endpoint(
            value="api.evil-c2.com",  # leak-scan: allow 合成 C2 夹具域名，基线既有值非案件真实值
            kind="domain",
            evidences=[Evidence(source="runtime-pcap", location="reference.json", scope=scope)],
            enrichment={"runtime": {"business_api_paths": ["/api/order"]}},
        )

        score = score_asset(endpoint)

        assert score.score == 0, scope
        assert not any(reason.startswith(("runtime-access", "business-path")) for reason in score.reasons)
    ...
    assert score_asset(direct).score == 40
