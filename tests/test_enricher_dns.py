...
def test_doh_fake_ip_is_quarantined_and_resolver_is_recorded(
    fake_requests: _FakeRequests, fake_lookup: _FakeBatchLookup
) -> None:
    """198.18/15 常由代理 Fake-IP 返回，不得进入真实 IP/托管查询。"""
    fake_requests.response = _FakeResponse(_doh_payload(["198.18.0.7", "198.51.100.21"]))
    fake_lookup.table["198.51.100.21"] = {
        "isp": "ISP B", "org": "Org B", "asn": "AS222", "country": "CN"
    }

    result = DnsEnricher().enrich(_ep("fake-ip.example.test"))

    assert result.ok is True
    assert result.data["ips"] == ["198.51.100.21"]
    assert result.data["rejected_answers"] == [
        {"value": "198.18.0.7", "reason": "benchmark_or_proxy_fake_ip"}
    ]
    assert result.data["resolution"]["resolver"].startswith("https://")
    assert isinstance(result.data["resolution"]["queried_at"], (int, float))
    assert result.data["resolution"]["independent_views"] == 1
    assert fake_lookup.calls == [["198.51.100.21"]]


def test_only_fake_ip_is_retained_as_negative_evidence_without_hosting_lookup(
    fake_requests: _FakeRequests,
    fake_lookup: _FakeBatchLookup,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """只有 Fake-IP 时仍是一次有效观测，但不得造公网 IP、托管归属或缓存阴性结果。"""
    fake_requests.response = _FakeResponse(_doh_payload(["198.18.0.7"]))

    def fake_gethostbyname_ex(name: str) -> tuple[str, list, list[str]]:
        return (name, [], ["198.18.0.8"])

    monkeypatch.setattr(dns_mod.socket, "gethostbyname_ex", fake_gethostbyname_ex)

    result = DnsEnricher().enrich(_ep("fake-only.example.test"))

    assert result.ok is True
    assert result.data["ips"] == []
    assert result.data["hosting"] == []
    assert result.data["resolution_status"] == "no_usable_public_a"
    assert result.data["rejected_answers"] == [
        {"value": "198.18.0.7", "reason": "benchmark_or_proxy_fake_ip"},
        {"value": "198.18.0.8", "reason": "benchmark_or_proxy_fake_ip"},
    ]
