...
def test_extract_windows_project_root_outside_users() -> None:
    ...
    """
    blob = b"\x00D:\\im_demo2\\appsdk\\sdk\\pool.go\x00"
    assert extract_paths(blob) == ["D:/im_demo2/appsdk/sdk/pool.go"]
    # 两段的项目根本身也要留下（它就是身份线索）
    assert extract_paths(b"\x00D:\\im_demo2\\appsdk\x00") == ["D:/im_demo2/appsdk"]

    cp = classify_path("D:/im_demo2/appsdk/sdk/pool.go")
    assert cp.tier == TIER_SELF_HOSTED
    assert cp.root == "d:/im_demo2"
    assert cp.identifier == "appsdk"
...
def test_classify_dependency_cache_is_third_party() -> None:
    ...
    assert classify_path("D:/im_demo2/appsdk/sdk/pool.go").tier == TIER_SELF_HOSTED
...
def test_real_windows_project_root_still_self_hosted() -> None:
    ...
    c = classify_path("D:/buildroot/appsdk/sdk/bootstrap.go")
    assert c.tier == TIER_SELF_HOSTED and c.identifier == "appsdk"
...
def test_sparse_build_env_is_kept_out_of_cross_case_dimension() -> None:
    ...
    got = C._build_environments(_env_report([
        ...
        {"root": "d:/buildroot", "identifier": "appsdk", "count": 26},
    ]))

    idents = [g["identifier"] for g in got]
    assert idents == ["Env0000-Aaaa-AV", "appsdk"], f"门槛没生效：{idents}"
