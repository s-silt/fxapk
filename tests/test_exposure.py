...
不研判漏洞、不给利用方向。数据来自 shodan / webcheck 已采集的被动 banner，对目标零流量。
"""
...
def test_assess_tech_stack_from_shodan_banner() -> None:
    ...
    names = {t["name"] for t in exposure.assess_tech_stack(shodan, None)}
    assert "Jeecg-Boot 低代码后台" in names   # http_title 命中
    assert "PHP" in names                      # cpe 命中
    assert "Spring / Java" in names            # product=tomcat 命中


def test_assess_tech_stack_from_webcheck() -> None:
    # webcheck 被动技术栈识别（Wappalyzer 类）：technologies[].name 并入 product / title 桶。
    webcheck = {"tech-stack": {"technologies": [{"name": "Laravel"}, {"name": "ThinkPHP"}]}}
    names = {t["name"] for t in exposure.assess_tech_stack(None, webcheck)}
    assert "Laravel" in names
    assert "ThinkPHP" in names
...
def test_tech_stack_is_identify_only_no_vuln_fields() -> None:
    ...
    jeecg = next(t for t in exposure.assess_tech_stack(shodan, None) if "Jeecg" in t["name"])
...
def test_assess_tech_stack_empty_safe() -> None:
    assert exposure.assess_tech_stack(None, None) == []
    assert exposure.assess_tech_stack({}, {}) == []
    # 无任何已知栈指纹（纯 nginx）→ 空列表。
    assert exposure.assess_tech_stack({"services": [{"port": 443, "product": "nginx"}]}, None) == []


def test_build_host_fingerprint_flattens_passive_banners() -> None:
    # 主机指纹拍平：shodan services 的 product / http_server / http_title / module / cpe + webcheck 技术名。
    fp = exposure.build_host_fingerprint(
        ...
        {"tech-stack": {"technologies": [{"name": "Laravel"}]}},
    )
    ...
    assert "laravel" in fp["product"] and "laravel" in fp["title"]  # webcheck 名并入
    # 坏输入安全：非 dict → 全空桶。
    empty = exposure.build_host_fingerprint(None, None)
    assert all(v == set() for v in empty.values())


def test_render_tech_stack() -> None:
    tech = exposure.assess_tech_stack({"services": [{"port": 443, "http_title": "Jeecg-Boot"}]}, None)
