...
_TSV = "\n".join([
    "c2.evil.example.com\tPOST\t/api/report\tokhttp/4.9\t1.2.3.4\t80",
    "c2.evil.example.com\tGET\t/api/config\tokhttp/4.9\t1.2.3.4\t80",
    "\tGET\t/no-host\t\t5.6.7.8\t80",          # host 为空 → 跳过
    "tracker.example.com\tGET\t/t\t\t9.9.9.9\t8080",  # UA 缺（列少）也解
    "",                                          # 空行跳过
])
...
def test_parse_http_fields() -> None:
    ...
    assert reqs[0].host == "c2.evil.example.com" and reqs[0].method == "POST" and reqs[0].uri == "/api/report"
    assert reqs[0].user_agent == "okhttp/4.9" and reqs[0].dst_ip == "1.2.3.4" and reqs[0].dst_port == "80"
    assert reqs[2].host == "tracker.example.com" and reqs[2].uri == "/t"
...
def test_to_endpoints_dedup_by_host() -> None:
    ...
    assert hosts == {"c2.evil.example.com", "tracker.example.com"}  # 一 Host 一端点（dedup）
    assert all(e.kind == "domain" for e in eps)
    assert all(e.is_cleartext for e in eps)  # ★复审 #6：明文 HTTP 端点定义上就是 cleartext
    c2 = next(e for e in eps if e.value == "c2.evil.example.com")
...
def test_host_normalization() -> None:
    ...
    reqs = [
        tshark_backend.HttpRequest(host="API.Evil.EXAMPLE.COM:8443", method="GET", uri="/a"),
        tshark_backend.HttpRequest(host="api.evil.example.com.", method="POST", uri="/b"),  # 尾点 → 同一 Host
        tshark_backend.HttpRequest(host="198.51.100.30", method="GET", uri="/c"),       # IP 字面量
    ]
    eps = tshark_backend.to_endpoints(reqs)
    vals = {e.value: e.kind for e in eps}
    assert vals == {"api.evil.example.com": "domain", "198.51.100.30": "ip"}  # 大小写/端口/尾点归一后 dedup 为一


def test_tab_in_field_defense() -> None:
    """★复审 #7：字段值内嵌 tab 致列溢出 → uri/ua/ip/port 不可信置空，host/method 仍保留、不产错域名。"""
    line = "evil.example.com\tGET\t/path\twith\ttab\tinUA\t1.2.3.4\t80"  # URI/UA 里的 tab 撑出 8 列
    r = tshark_backend.parse_http_fields(line)
    assert len(r) == 1 and r[0].host == "evil.example.com" and r[0].method == "GET"
...
def test_run_tshark_mocked_subprocess(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert len(reqs) == 3 and reqs[0].host == "c2.evil.example.com"
...
_DEC_TSV = "\n".join([
    ...
    "\t".join(["", "api.evil.example.com", "", "POST", "", "/zj/api/user_login", "", "okhttp/4.9", "1.2.3.4", "443"]),
    # HTTP/1.1-over-TLS 行（host 在 http.host）
    "\t".join(["secure.evil.example.com", "", "GET", "", "/config", "", "Dalvik/2.1", "", "198.51.100.48", "443"]),
    # 无 host/authority → 跳过
...
def test_parse_decrypted_fields() -> None:
    ...
    assert reqs[0].host == "api.evil.example.com" and reqs[0].method == "POST"  # h2 :authority/:method 合并
    assert reqs[0].uri == "/zj/api/user_login" and reqs[0].user_agent == "okhttp/4.9"
    assert reqs[0].dst_ip == "1.2.3.4" and reqs[0].dst_port == "443"
    assert reqs[1].host == "secure.evil.example.com" and reqs[1].method == "GET" and reqs[1].uri == "/config"
...
def test_run_tshark_decrypt_mocked(monkeypatch, tmp_path) -> None:
    ...
    assert len(reqs) == 2 and reqs[0].host == "api.evil.example.com"
...
def test_decrypted_to_endpoints() -> None:
    ...
    assert {e.value for e in eps} == {"api.evil.example.com", "secure.evil.example.com"}
    for e in eps:
    ...
    api = next(e for e in eps if e.value == "api.evil.example.com")
    assert "TLS 解密" in api.evidences[0].snippet
    assert "https://api.evil.example.com/zj/api/user_login" in api.evidences[0].snippet  # https + 解密 URL
...
def test_cleartext_to_endpoints_unchanged() -> None:
    ...
    c2 = next(e for e in eps if e.value == "c2.evil.example.com")
    assert c2.evidences[0].source == "runtime-tshark"
    assert "明文 HTTP" in c2.evidences[0].snippet and "http://c2.evil.example.com" in c2.evidences[0].snippet
...
_CRED_TSV = "\n".join([
    ...
    "\t".join(["", "api.evil.example.com", "", "POST", "", "/zj/api/user_login", "", "Bearer secrettoken1234567890", "", ""]),
    # HTTP/1.1 请求：Cookie 在 http.cookie（col8）
    "\t".join(["secure.evil.example.com", "", "GET", "", "/config", "", "", "", "sid=abc; token=xyz", ""]),
    # 无凭据头（filter 理论不产出，防御性）→ 丢弃
    "\t".join(["nohdr.evil.example.com", "", "GET", "", "/x", "", "", "", "", ""]),
    "",
])
...
def test_parse_decrypted_credentials() -> None:
    ...
    assert creds[0]["url"] == "https://api.evil.example.com/zj/api/user_login" and creds[0]["method"] == "POST"
