...
**境外**服务器→**被动定位真实源站 IP + 提取独特标识**（RDAP / DNS / ASN / 证书透明度 / 历史解析穿透 CDN），不主动探测、不接触目标。

> 性质声明：本项目为**个人安全研究 / 测试**用途；分析中如发现涉诈线索，可提供给**相关部门**依法处置。文中"调证 / 办案 / 协查"等仅说明合法取证路径，不代表任何官方身份。
...
| `backend_credential` | 硬编码后端 / 管理凭据（Basic-Auth / DB 连接串 / JDBC / 云 AccessKey）→ 供有权机关依法登录取证（被动静态提取，不向任何远程服务器发起连接） |
...
| `ios_plist` ★ | （iOS IPA）解析 `Info.plist` → 冒充品牌（显示名）/ URL scheme 声明 / ATS 明文 HTTP / 探测支付宝·微信 / 敏感权限用途 |
...
实名）；**境外**服务器（难直接调证）→ **被动定位真实源站 IP + 提取独特标识**：经 RDAP / whois /
DNS / ASN / 证书透明度（crt.sh 子域）/ 历史解析穿透 CDN，识别真实源站归属、技术栈指纹与关联子域用于
串案，**全程被动、对目标零流量、不主动探测 / 不攻击**。

> 硬编码后端凭据（被动提取）：`backend_credential` 从**已扣押样本**里抠出它自己硬编码的后端 / 管理
> 凭据（App 自带的凭据十有八九就是那台服务器在用的），供有权机关依法登录取证——**只做静态提取，不向任何远程服务器发起连接**。
...
- **隐私安全**：digest **默认明文**（取证查看需要看到钱包私钥 / 凭据等实际值）；要把摘要喂可能经云端模型处理的 agent 时用 `fxapk digest --redact`，对高敏物证（钱包私钥 / 助记词、后端凭据、运行时登录态、受害人 PII、加密配方）脱敏，明文始终在本地完整 `report.json`。联网富化默认仅查「建议调证」端点、缩小查询足迹；web-check 走**本地实例**；ip-api 明文 HTTP 风险见下方隐私提示。
...
本工具仅用于**授权的反诈调证 / 安全研究**，只做分析与线索提取，**不提供任何针对第三方服务器的攻击 / 漏洞利用 / 主动探测能力**；
加固只识别不脱壳（脱壳为可选的真机取证步骤，只对**样本自身**在取证机上运行时观测，需操作者自备授权环境）。
境外服务器只做**被动归属**（RDAP / WHOIS / ICP 备案 / ASN / DNS / 证书透明度），定位真实源站 IP 与独特标识，
