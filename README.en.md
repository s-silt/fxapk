...
1. **`analyze` goes online by default.** It never touches the target's servers, but it does take the
   ...
   **which amounts to disclosing to third parties what you are analyzing**, and you
...
> `fxapk probe-leads` and
> `fxapk pcap-leads` print lead
...
> paste them there**.
> `corpus link-discover`, `link-explain`, and `link-groups` are different: they default to
> `--evidence-values omit` and expose only run-local aliases plus allowlisted fields. Selecting
> `raw` explicitly restores family, sample, and anchor values and emits a stderr warning.
> `link-labels-validate`, `link-evaluate`, `link-readiness`, and `link-train` emit aggregate-only
> results. These are command-specific safe projections; they do not broaden `digest` redaction.
>
...
Normal analysis, deterministic candidates, and the readiness gate do not need an ML stack. Install
`pip install 'fxapk[ml]'` (or `pip install -e '.[ml]'` from source) only after `link-readiness`
returns `ready` and you intend to train the experimental challenger locally.
