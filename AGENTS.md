...
**深度归因（拿到后端域名/IP/标识符后做服务器归因 + 调证报告）**：按深度归因打法执行——证据分级 + 对抗式核验 + 绝不编造（web-check/Shodan 可实查，其余 key-gated 给语句）+ 辖区驱动 P0–P5 调证优先级 + 固定输出《技术侦查与调证建议报告》A–L。（手册在私有 handoff 目录 `fxapk-handoff/claude/doc/打法/deep-attribution-playbook.md`，不入库。）
