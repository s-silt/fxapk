...
**两件套（操作机已装，优先用，别空跑外部付费源）**：① `fxapk`（本仓库，APK 取证→端点/IP/标识符+富化+图谱+台账）；② `web-check`（`lissy93/web-check` 自托管 OSINT，无需 key=实测级）。**先设 `FXAPK_WEBCHECK_URL=http://localhost:3000`**（按本机端口），fxapk 跑富化时就自动对建议调证端点拉 web-check（DNS/SSL/头/技术栈/端口/威胁/子域/邮件…）进辖区分流与境外源站归属。

**标准动作（先跑命令、据产物决策，别空想）**
1. `fxapk analyze <apk> --online --out out` → `fxapk digest out/<名>.json`。
2. 决策以 digest 为主；仅在补「国内登记主体端点」「境外源站归属细节」时翻 `report.json` 全量。
...
**动态：PCAP-first 保底，明文优先被动解密，探针是可选旁路（非必做）**：动态已转向**零注入 PCAP 底座**——`fxapk capture/auto` 起 floor PCAP，拿到接入节点 / SNI / QUIC Initial / socket 归因等被动证据即算**有效产出**（不再以"抓到明文"为唯一成功标准）。要明文时**优先走被动链路**：TLS Key Log + tshark 解密、socket 归因把流量落到进程 / UID / PID。★**不再要求"抓不到就必须写 frida 探针"**——Frida / native hook 降为**可选旁路**，仅当被动手段确实不够、且在 `--mode authorized-active` 授权下按需用；动手写侵入式 hook 前先自问"被动 PCAP + keylog 是否已够"。
...
2. **动态**（有设备）：floor PCAP 保底拿到 ≥1 接入节点 / SNI 即算有产出；要明文优先走**被动**解密（TLS keylog + tshark）。侵入式探针**不是必做项**——被动手段够就不写；只有连 PCAP 底座都没起来才算「零产出」没走完（见 §0.5 动态口径）。
3. **富化 / 判型**：每个「建议调证」端点判辖区 + 判前端/落地；富化源命中太少 = 源没跑全，别据残缺证据下结论。
4. **降噪**：剔反诈拦截页 / 大厂共享 / CDN 边缘，别当落地机。
5. **串案**：独特字值（appkey / 证书 / CNAME 模板）跨案自查。
6. **闭环校验（收工前必过）**：① 每个 lead 有明确调证落点（向谁调、取什么证据）；② 动态非零产出；③「待核」项要么核清、要么**显式**标「待核 + 为什么 + 下一步」，不能默默漏掉。
...
1. **第①遍·归属** → 判服务器**辖区**（国内/境外/未知）：rdap/whois/dns/asn/icp/webcheck。
...
| webcheck（本地自托管实例） | 归属/地理/技术栈/端口/子域等 OSINT | 被动·对目标零流量 |

**技术栈指纹（`exposure`，纯映射·零网络·零 payload·默认开）**：把 shodan/webcheck 已采集的被动 banner 映射到**技术栈/后台框架指纹**（PHP/Laravel/ThinkPHP/Spring/致远/泛微/通达OA…），作为**同后台=疑同团伙的串案信号**——**仅识别、只用于串并**，不做漏洞方向研判、不利用。
