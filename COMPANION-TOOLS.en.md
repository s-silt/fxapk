...
still run, and unconfigured built-in sources are recorded as `disabled` in the source-status output.
DayDayMap is explicitly marked as an external companion below: its variables are reserved for local key
management, but fxapk core neither calls it nor emits a DayDayMap `source_status`.
...
| `FXAPK_DAYDAYMAP_KEY` / `FXAPK_DAYDAYMAP_KEY2` | DayDayMap (external companion; not built into core) | daydaymap.com |
